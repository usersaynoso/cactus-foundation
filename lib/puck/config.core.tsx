// Puck component config, CORE BLOCKS ONLY — safe for both editor (client) and
// RSC render paths. No hooks, no browser APIs. Type imports only from
// @puckeditor/core.
//
// **This file must never import lib/puck/module-components.ts.** That map holds
// a static import of all 118 module block CLIENT components, and this file is
// reached from lib/puck/config.rsc.tsx, which every public page renders through
// (app/(public)/layout.tsx). One import edge therefore put every installed
// module's client code — 3D viewers, abandoned-cart trackers, product checks —
// into the browser bundle of every page on the site, whether the page rendered
// any of them or not. It measured 690KB of compressed JavaScript on a homepage
// that shows eleven images and some text.
//
// Nothing was ever rendering those client components on a published page: every
// one of the 118 blocks ships an RSC half, and config.rsc.tsx overrides all of
// them. They were dead weight, kept alive purely by the import.
//
// lib/puck/config.tsx is now a thin wrapper that adds the module blocks back for
// the editor and re-exports this file's API unchanged, so `@/lib/puck/config`
// means exactly what it always did to both editors and to module code.
// lib/puck/config.core.test.ts holds the graph guard.
...
export function withResponsiveVisibility(def: any): any {
...
function PhoneBlock(props: any) {
  ...
  const linkStyle: React.CSSProperties = {
    ...
    fontSize,
    ...(PHONE_WEIGHT_MAP[weight] ? { fontWeight: PHONE_WEIGHT_MAP[weight] } : {}),
    ...(transform && transform !== 'none' ? { textTransform: transform as React.CSSProperties['textTransform'] } : {}),
...
export function SiteLogoRsc(props: any) {
  const { id, logoUrl, logoUrlDark, imageUrl, imageUrlDark, siteName, cellHeight, cellHeightShrunk, logoHeight, logoHeightShrunk, showTextWithLogo = 'false', showIcon = 'true', textColor, align, nudgeY, homeUrl = '/' } = props
  ...
  // Vertical nudge, from the same helper as the client half - a transform, so
  // it never re-flows the header row it sits in.
  const { transform: nudgeTransform, css: nudgeCss } = siteLogoNudge(id, nudgeY)
  const style: React.CSSProperties = { display: 'flex', alignItems: 'center', justifyContent, gap: '0.5rem', fontWeight: 700, fontSize: '1.125rem', color: textColor || 'var(--color-text)', textDecoration: 'none', ...(nudgeTransform ? { transform: nudgeTransform } : null) }
  if (lightSrc) {
...
export const puckConfig = {
  ...
  components: (() => {
    ...
    Phone: {
      label: 'Phone',
      fields: {
        number: { type: 'text' as const, label: 'Phone number (dialled - use the full +44… form)' },
        ...
        letterSpacing: { type: 'custom' as const, label: 'Letter spacing', units: ['em', 'px'], render: UnitValueField },
        fontFamily: { type: 'custom' as const, label: 'Font', render: ({ value, onChange, field }: any) => <SiteFontField value={value} onChange={onChange} label={field.label} /> },
        color: { type: 'custom' as const, label: 'Colour', render: ({ value, onChange, field }: any) => <SiteColourField value={value} onChange={onChange} label={field.label} allowManual /> },
        ...
        underlineOffset: { type: 'custom' as const, label: 'Underline offset', units: ['em', 'px'], render: UnitValueField },
        padding: paddingField,
        ...STICKY_FIELDS,
        ...aosFields,
      },
      ...
      resolveFields: (data: any, { fields }: any) => {
        ...
        if (p.underline !== 'underline' && p.hoverUnderline !== 'underline') {
    ...
    MenuBlock: {
      ...
      fields: {
        ...
        itemFontFamily: { type: 'custom' as const, label: 'Font', render: ({ value, onChange, field }: any) => <SiteFontField value={value} onChange={onChange} label={field.label} /> },
    ...
    SiteHeader: {
      ...
      fields: {
        ...
        itemFontFamily:   { type: 'custom' as const, label: 'Nav font', render: ({ value, onChange, field }: any) => <SiteFontField value={value} onChange={onChange} label={field.label} /> },
...
// Empty here by design: this file must not import the module block map (see the
// header comment). lib/puck/config.tsx merges the real ones back in for the editor.
const footerModuleBlocks: Record<string, any> = {}
...
// Empty here for the same reason as footerModuleBlocks above.
const headerModuleBlocks: Record<string, any> = {}
