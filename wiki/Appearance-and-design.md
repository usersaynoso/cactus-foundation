...
| **Account Login** | The sign-in page itself, at /account/login. Your design goes around the form, which arrives in the **Content slot**. |
| **Account Login Modal** | The panel that opens over the page when a visitor clicks the sign-in icon in your header. Same idea: your design fills the panel, the form arrives in the **Content slot**. |
| **Document footer** | The strip repeated at the foot of *every page* of a printed document - your registration small print, a page number, a rule. Shared by everything your site prints: invoices, credit notes, proformas, quotes and purchase orders. |

The two **Account Login** types are a pair, and they are separate on purpose. One is the sign-in page a visitor lands on; the other is the panel that floats over whatever they were reading, which the sign-in button in your header opens if you have set it to. A design that suits a whole page rarely suits a small panel, so each gets its own. In both cases the sign-in form itself is not a block you place - it carries the "take me back where I was" link, the sign-in link from an email and the staff shortcut, none of which a block dropped in by hand would know about - so you mark where it goes with the **Content slot** block and design everything around it. Publish a modal layout and it takes over the whole inside of the panel: the heading and the "Create an account" line the panel draws for itself step aside, so put your own in if you want them. Nothing is published for you, and until you build one both the page and the panel look exactly as they do today.

**Document footer** is worth a word, because it is the odd one out. It is not a page: it is drawn into the bottom margin of a PDF by the thing doing the printing, which is why it repeats on every page where a footer block on the document itself would print once, after the last line. The blocks you can put on it come from whichever module does your paperwork - Shop contributes its small print, page number, notice and rule blocks - so on a site with no such module you get the ordinary text and logo blocks, which make a perfectly good single line at the foot of a page. Design it once and it turns up on all of it: the invoice, the credit note, the proforma, the quote and the purchase order. It takes your site's own colours and typefaces, so a rule drawn on it is the same rule the document above it draws, and it makes its own room: a tall footer opens up the document's bottom margin rather than printing over the last few lines. Nothing is published for you: until you create one, anything your site already prints looks exactly as it did.

Some modules add their own layout types too, shown as an extra tab alongside the built-in ones (with a second row of tabs underneath for that module's own sub-types). At the moment: **Directory** (Category, Entry), **Gazette** (Listing, Post), **Boards** (Board, Thread), **Purchase Orders** (Purchase order document, Returns note), and **Shop** (Shop Home, Category, Collection, Product, Checkout, Confirmation) - so you can design the look of a directory category page, a blog article, a forum thread, or a shop product the same way you design your header or footer. See each module's own wiki page for what its sub-tabs mean.
...
**Nudging the logo up or down.** Vertical centring is the sensible default, and it is what your header does. It is also, now and then, a pixel or two out - a logo with a lot of empty space baked into the image, or a wordmark with descenders, can look low even when it is dead centre. The Site logo block has a **Nudge down (px, minus = up)** box for exactly that: type `3` to move it three pixels down, `-3` to lift it three pixels up. It is set per screen size, so a nudge on desktop need not follow you onto phones. It moves only the logo - the header keeps its height and everything beside it stays put - and leaving the box blank changes nothing at all.
...
Want a strip of header colour left above the search box, rather than the search box jammed against the top of the screen? Type a slightly smaller height, then switch on **Hide what scrolls away once pinned**. Without it, the bottom of your logo lingers in that strip like a guest who has missed every hint. With it, anything that started in the part that scrolls away fades out the moment the header pins, and fades back in when the visitor scrolls back to the top. The phone bottom bar is never touched.
