...
This is a different system from admin **Users** (see [Managing users](Managing-users)) - members never have access to your admin area, and admins never appear in the members directory. The two never mix.

---

## Turning it on

Go to **Members → Settings** in the admin and tick **Members system enabled**. Nothing changes for visitors until you do this.
...
- A **Members** section in your admin sidebar.

---

## How people sign in

Members can sign in three ways, and you choose which are switched on (**Members → Settings → Access control**):
...
**Members → Settings → Registration** controls who can sign up:

- **Open** - anyone can register.
- **Invite only** - people need an invite link, which you generate from **Members → Invites**. Each invite can be used once and expires after however many days you set.
- **Approval required** - anyone can register, but their account waits in a queue (**Members → Pending approval**) until an admin approves it.
...
Once signed in, members manage their own account from a few sections you can individually switch on or off (**Members → Settings → Account sections**):
...
Every member gets a profile page at `/members/<username>`. You control who can see it (**Members → Settings → Account sections → Public profile visibility**):
...
The **Members** section in your admin sidebar gives you:

- **Overview** - at-a-glance counts and anything needing your attention.
- **Members** - a searchable, filterable list with bulk actions (suspend, approve, trust, delete).
- A **member detail page** for each person - edit their profile, suspend or approve them, mark them as trusted, revoke their sign-in sessions, reset their password and two-factor setup, trigger a data export on their behalf, leave internal notes, and see a full history of admin actions taken on their account.
- **Pending approval** - the queue when registration mode is set to Approval required.
- **Invites** - generate and revoke invite links.
- **Email templates** - customise the wording of every email members receive (welcome, verification, security alerts, and so on), with a merge-tag list and a test-send button. Reset any template back to the default whenever you like.
- **GDPR** - a dashboard of consent records, data export requests, and pending deletions, all in one place.
- **Settings** - everything above, plus site-wide access control (see below).

---

## Making the whole site members-only

**Members → Settings → Access control → Site-wide members-only mode** locks your entire public site behind sign-in - visitors must be a member to see anything. You can list specific pages as exceptions (for example, your homepage or a "why join" page), and admins always get through regardless.
