...
Members who arrived through the checkout don't start empty either. Once the email address has been verified, guest orders placed at that address are matched to the account, and the name and organisation from the most recent one are copied onto it - and the same happens when somebody already signed in places an order. It isn't a one-off at the moment of matching: any visit to the account or the order history will fill in a blank name or organisation from the member's own order history, so a member left blank by an earlier version doesn't stay that way. Only ever into blanks: anything the member has typed on their own account page wins, so a correction is never quietly undone by an older order.
