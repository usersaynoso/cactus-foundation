...
| `CLOUDFLARE_ZONE_ID`, `CLOUDFLARE_PURGE_API_TOKEN` | Clearing cached pages the instant they are edited, when **Settings → Speed → Keep ready-made copies of your pages** is on. Both can be entered in the admin, on that same Speed tab. The page cache works fine without them - an edited page simply waits out its window. The token needs the **Zone → Cache Purge** permission. |
