...
And both carry **returns**: `returnable` and `non_returnable_reason` on Products, **Returns** on Variations. These are the one place a blank cell is not simply "off" - it means "nothing said", which on Products leaves the thing returnable and on Variations leaves the row following its product. Only a plain `false` (or `No`) refuses. So a sheet that never touches those columns changes nothing, and marking a whole made-to-order range is a fill-down rather than an afternoon.
