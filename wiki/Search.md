...
**Search Box** (~36 options): behaviour (mode, minimum characters, debounce, result count, grouping, hotkey, autofocus, results path), per-content-type toggles, appearance (field / field with button / icon button, size, corner style, outlined/filled/minimal, accent colour token, width and alignment), and dropdown options (width, products as rows / cards / designed shop cards, card columns, thumbnails, excerpts, type badges, prices, match highlighting, view-all and empty text).
