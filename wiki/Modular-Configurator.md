...
- **Build a layout** starts with the ready-made shapes drawn to scale, each priced in the options already chosen, and **Design your own**. Choosing one starts the builder - the 3D view only loads at that point:
  ...
  - **Design your own** opens straight onto **Choose your first unit**, and leaves the ready-made shapes out of the way.
  ...
  - **Undo** and **Reset layout** (empties the layout and offers the first unit again), **Start from a shape** (not shown when designing your own), and the layout's own options (fabric, frame) with swatches.
