...
**Press any of those labels and the whole story opens.** Every delivery, open, followed link and bounce the message has had, in order, each with the exact moment and the internet address it came from. The address is the useful part: an "open" three seconds after delivery from a cloud provider's network is the recipient's email security looking the message over, and the person themselves tends to turn up later, from somewhere ordinary. Brevo's own logs show the same thing, and this saves the trip. The address on a delivery is the email service's own machine handing the message over, which is worth knowing before anybody decides it looks familiar. Brevo is asked for the addresses the first time the history is opened and they are kept from then on; if it cannot be reached the list still opens, with a line saying the addresses could not be fetched this time.
...
Once somebody is on it, that address appears in their rail under **Team inboxes**, named after the colleague rather than after the address - **Sam Blake**, not `sam@`, because that is how anybody covering Sam's post thinks of it. It sits flush with every other name on the rail; there is no arrow in front of it and no indent. **Press the name and their folders open out underneath it** - going to somebody's post is the intent to look at their post, and the folders are half of what is in it. Put the pointer on the row and the coloured dot turns into a **&rsaquo;** you can press to fold them away again while you are in there; go somewhere else and come back and they are showing once more.
...
Only the buttons that would actually change something are drawn. Pick six conversations you have already read and there is no read button to press; pick six that are all open and there is no **Reopen**. One out of six is enough to bring a button back, since that is exactly when you want it.

**What is picked, the open conversation obeys too.** Pick six, open one of them to check it is the right pile, and the bin, the junk sign, the clock and **Done** at the top of that conversation act on all six rather than on the one you happen to be reading. It used to file exactly one, with the other five still ticked behind it and nothing on the screen admitting it. The button says how many it is about to take with it when you hover over it, the question the junk sign asks names the number, and the five seconds afterwards put the lot back. Handing a conversation to a colleague is deliberately still one at a time - whose desk something lands on is a decision per conversation.

**The bar stays put while you scroll.** Pick the top of a run, scroll down, shift-click the bottom of it, and the buttons are still there at the top of the list rather than a screen and a half above your head. The list slides underneath it.

**The list grows, it does not turn pages.** Twenty-five conversations to begin with, and at the foot of them a button saying how many more there are - press it and the next twenty-five appear underneath the ones already there, so you end up with fifty in one list rather than a second page with the first twenty-five gone. Whatever you had picked stays picked, and whatever you had open beside the list stays open. It stops at five hundred, at which point the list is telling you to search for the thing rather than scroll to it.

**Snooze is the exception, and it is always there.** It opens the same list of times the clock on a single conversation opens - later today, tomorrow morning, next week, or a day and time of your own off the calendar - with the day and hour each one lands on written beside it. Every one of the picked conversations goes to sleep until then and comes back at the top of the list. It stays on the bar even when they are all already asleep, because choosing a new time for something is a real change rather than writing down what was written already. Bringing a pile back early is **Reopen** beside it, not in here.

**The junk sign** asks before it does anything. It tells you the lot are about to go into a spam folder - yours, or a colleague's where the address is their own rather than the team's - and asks whether you would also like to turn those senders away in future, listing them so you can see who. **No, just move them** does the move and leaves the front door alone. **Block them all** does both. **Cancel**, the cross in the corner and Escape all leave every one of them exactly where it is, which is what you want the moment a finger lands on the wrong button in a toolbar. Blocking means nothing further from them reaches an inbox on this site, shared or personal - it goes straight in the spam folder instead, so you can still see what they sent. Nothing already here is touched, and you can let anybody back in from the Spam folder or the inbox settings. The blocking half only appears for somebody allowed to answer messages, since shutting the front door changes what everybody on the site receives - and where there is nobody in the pile to turn away, or you are not the one who may, it asks the shorter question instead: move them, or cancel.
...
**On a channel that can refuse people itself - the phone, say - the same question is asked about them rather than about an address.** *Would you also like to stop them getting through on Phone in future?* **Block them** does the junk and the refusing; **No, just move it** does the junk alone. It is the same decision in the same place, which is the point: refusing a caller happens for the same reason refusing an address does, and it should be asked after the junk has been dealt with rather than offered as a button somebody can lean on.
...
- **A caller is blocked by marking their conversation as junk**, if the channel they came in on can do it. Press the no-entry sign at the top of the conversation and the question that comes up asks whether you would also like to stop them getting through in future; **No, just move it** does the junk and leaves them alone. There is no longer a standing **Block them** button in the header - it was the one control up there that changed what a stranger gets when they next try to reach you, it sat in front of everybody all day, and it was pressed by mistake. On the phone, blocking means their next call is dropped the moment it arrives: nobody's phone rings, no message is taken and no alert goes out. It deletes nothing they have already said, and once somebody *is* blocked an **Unblock them** button appears at the top of that conversation, which is the only time it is on the screen at all. See [Blocking a caller](Twilio#blocking-a-caller).
