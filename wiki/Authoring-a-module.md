...
The site registers exactly **one** cron with Vercel: `/api/cron/dispatch`, hourly. That dispatcher reads every installed module's `cronJobs` (and core's own) and calls the ones that have come due since they last ran, recording each job's last run in the core `CronRun` table.
...
- **Your schedule is honoured to the tick, not to the minute.** The dispatcher wakes hourly on a paid plan, so `30 6 * * *` runs at the 07:00 tick. On Vercel's Hobby plan the dispatcher itself only runs once a day, so everything runs once a day - as it always did there.
