...
Run it on demand from the button, or let it run itself weekly (Mondays, 4am, when it is least likely to be in the way). A crawl is bounded by a time budget as well as a page limit, so a large site reports a partial run rather than failing outright. History is kept per run with per-page issue lists. The page limit per crawl is configurable in settings (default 50).
...
- **A Markdown twin of every page** - the same page at the same address with `.md` on the end. `/about` also answers at `/about.md`, `/task-chair` at `/task-chair.md`, and the home page at `/index.md`. No menus, no buttons, no scripts: the words, the headings, the links, the questions the page answers and - for a product - the price, the stock, the specification, the variations and whether there is a 3D model of it.

**The questions come too.** Whatever your product, category and collection pages answer in their FAQ section is written into the copy as well, merged exactly as the page merges it: the item's own questions first, then its category's, then its parent categories', then the shop-wide list, with a nearer answer beating a further one and a level that has been told not to inherit stopping the walk. These are the most quotable lines you have - what it weighs, whether it comes built, how long delivery takes - and until now they were the one thing on a product page an assistant reading the copy could not see. Switch the shop's FAQs off and they are not in the copies either.

**The specification is the one on the page.** The copy lists the attributes you flagged for the product page's Specification panel, under the names the panel shows and in the order it shows them. It used to list every attribute a product carried, flagged or not, which meant supplier catalogue codes, internal reference numbers and whatever else lives back there went out to every assistant that fetched the file, while the page itself showed none of it. If a product has nothing flagged for the panel, its copy has no specification table - same as the page.
...
**An update rebuilds the copies on its own.** When a Cactus update changes what goes into a copy - a new section, something that should never have been in there - every existing copy is out of date, and the overnight job used to skip them all: it only ever asked "has this page been edited since?", and a product nobody has touched in a year never had been. So an install could take an update, run the new code, and go on serving copies written by the old one indefinitely. It now also asks which version wrote each copy, and rewrites anything written by an older one, a night or two at a time until it has caught up. Nothing to press. **Rebuild everything** is still there if you would rather not wait.

**Nothing works until you have rebuilt once.** A fresh install, or an install that has just switched these on, has no copies built yet - so the index lists nothing, the `.md` addresses answer "not found", the agent endpoint says it has nothing to show and no page gets its breadcrumbs. The screen says so in as many words when it happens. Press **Rebuild everything** once and the overnight job keeps up on its own after that.
...
- The Markdown twins are materialised by `lib/ai/materialise.ts` - the weekly `cron/ai` job, the admin button, or a summary being saved - never on the request. Measured on a live catalogue: a hundred products with their variations, attributes and 3D models take about a second and a half, so a twenty-thousand-product shop rendered per fetch would be a bill rather than a feature. A rebuild compares each entity's `updatedAt` against the stored `source_updated_at` and skips what has not moved, stops at a forty-second budget rather than being killed mid-write, and only prunes after a complete pass. That budget is not a number of this module's choosing: a module route cannot set its own `maxDuration`, core's shared dispatcher for module API routes sets one ceiling of sixty seconds, and core's cron dispatcher then gives a job whatever is left of its own sixty-second tick. A budget set above that is not a longer run, it is a run that never finishes and is recorded as a failure, leaving a large catalogue silently short - which is what a four-minute budget between 0.1.12 and 0.1.13 would have done to any site big enough to need it. The same applies to the site audit, which budgets thirty seconds and will not start fetching a page it cannot also finish.
