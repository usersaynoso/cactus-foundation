...
Writing out takes a few minutes, and the hub checks on the hour. So a message sometimes reaches the inbox before its words do; the words are added to the same message when they arrive, and the conversation does not jump back to the top of your list for it - nothing new was said, it merely became readable. If Twilio cannot type a recorded call up, the audio still stays attached and the call log says the transcription failed.
