...
**The basket gets a tab of its own.** Everything brought over from the basket sits under **Waiting** in the side panel - a proper list with pictures, prices and sizes, not the cramped strip it used to be - and each thing goes into the room with a tap, or all of them at once with one button. Changed their mind about something? It comes off the list without going in. Anything a redrawn room can no longer hold waits in the same place rather than being thrown away. The tab only appears while something is actually waiting, and the count on it says how much.

**On a phone.** The side panel now pulls up like the sheet in a maps app - a tug on the handle gives the catalogue or the waiting list most of the screen, and placing anything drops it back down so the room shows what just arrived. Search and category sit side by side to spend less of the screen on furniture-finding and more on furniture.
