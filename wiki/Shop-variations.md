...
Nothing to do. As soon as a product has options or personalisation, its page picks them up on its own: the option choosers and personalisation fields appear above the basket button, the price keeps up with what the customer has chosen, and the photo swaps when their choice has one of its own. The add-to-cart button stays put until they've made a valid, in-stock choice and filled in anything required. As the customer picks their way through, any choice that would lead nowhere - a combination you don't offer, or one that's sold out - simply drops out of the list rather than sitting there greyed out, so every option still on screen is one they can actually buy. (A choice they've already made stays put even if a later change makes it a dead end, so the choosers never blank out from under them.)
