...
- **Import / export** downloads every variant as a spreadsheet and lets you upload one back - one row per variant, with its options, SKU, **Sale SKU**, price, stock, **Min Qty**, barcode and weight. A blank Min Qty cell means the variant follows the product's own smallest order, and a 1 means the same thing, so leaving the column alone changes nothing. The extra price types travel too: **Sale Price, RRP, Trade Price and Cost Price** each get their own column beside the ordinary price, so a big change to any of them is a spreadsheet job rather than a variant-by-variant one. A blank price cell means the variant simply hasn't got that figure; fill it in and upload to set it. Handy for a big price change or setting up lots of variants at once. Create the parent products in the shop first; the spreadsheet fills in their options and variants. The download asks which columns you want first - everything is ticked to start with, and you can cut it down to, say, just the variant code and price. **Variant ID** always comes along and can't be unticked: it's how each row finds its way back to the right variant when you upload the file again. The option and value columns come as one tick, as do any extra columns other parts of the shop add. Your choice is remembered on that computer for next time.
