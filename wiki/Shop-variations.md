...
There is a **Min qty** column beside Stock, for a combination you will not sell one of - the size that only ships in pairs, the finish that comes in boxes of ten. Fill it in and the product page's quantity picker opens on that number the moment the shopper settles on that combination, refuses to go below it, and says so in a line under the button; the basket's picker stops there as well. Leave the box empty and the combination simply follows whatever the product itself says on its **Stock & delivery** tab, which is what the greyed-out figure in the empty box is telling you - so a minimum that applies to the whole range is set once on the product rather than typed across three hundred rows.
