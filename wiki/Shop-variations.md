...
You'll then get a grid with one row per combination. Set each one's price, stock, SKU and photo, or use the **Fill every row** boxes above the grid to do the lot in one go. Untick the **On sale** box to take that combination off sale without deleting it, tick **Image up front** or **3D up front** to put its photo or its model on the product page before anyone has chosen anything (see [below](#showing-off-a-finish-before-anyone-has-chosen)), or use the **Delete** button at the end of the row to be rid of it for good. Click a combination's name (or the little arrow beside it) to open the row up and see every photo on it at a decent size - from there you can pick which ones go up front, make a different one its main photo, or take a single photo off without clearing the lot - handy when a big pile of options throws up combinations you'll never actually sell. To clear out a whole batch at once, tick the box at the start of each row you want gone (or the box in the header to select the lot), then press **Delete selected** - rather than working down the list one Delete at a time. When there are more columns than fit, the grid scrolls sideways and the first column, the one naming each combination, stays put so you never lose track of which row you're on.

The **Returns** column has three settings rather than a tick, because a blank has to mean something. Left on **As product** - where every row starts, and where nearly all of them should stay - the combination follows whatever the product itself says about returns, so a made-to-order range is marked once on the product rather than three hundred times here. Set it to **Returnable** or **No returns** only where one combination genuinely differs from the rest: the single stock finish on an otherwise bespoke range, or the one made-to-order finish on a stock one.
...
There is a **Min qty** column beside Stock, for something you will not sell one of - the size you will not send fewer than two of, the finish that is not worth setting up for fewer than ten. It is a floor rather than a batch size, so a customer can order eleven of that finish; they just cannot order nine. Fill it in and the product page's quantity picker opens on that number the moment the shopper settles on that combination, and says so in a line under the button. **The count is across the product, not the combination**: a minimum of four is met by four of one colour or by one each of four colours, so a customer buying a mixed set is never told to take four of the same thing. Only when the basket still falls short of the number is it held back at the checkout, with a line saying how many more are needed. Leave the box empty and the combination follows whatever the product itself says on its **Stock & delivery** tab, which is what the greyed-out figure in the empty box is telling you - so a minimum for a whole range is set once on the product rather than typed across three hundred rows. Where two combinations disagree, the larger figure wins for the basket as a whole.

With the sale price switched on, a **Sale SKU** column sits beside the ordinary SKU one too. Fill it in when a supplier issues a separate code for a discounted variant - order under that code while the offer runs, and the variant's own SKU is left exactly as it is, so anything matched against your usual stock list still finds it.

Because each variation now carries its own price, the product's own **Pricing** tab steps back: once it has variations, the price boxes there are put away behind a short note pointing you to this grid instead. A figure set on the product as a whole would never be shown to anyone - the shop prices the product from its variations - so there's nothing to fill in there and no risk of the two disagreeing. Anything already typed on that tab is kept, just not editable, so nothing is lost if the last variation is ever removed. Out on the shop - product pages, and every listing and grid of cards - a product whose variations differ in price shows as **From £** its cheapest one, rather than a single figure, so a shopper sees where it starts at a glance. Price every variation the same and the "From" is dropped: there's nothing to count up from, so the shop simply states the price, and it goes back to saying "From" the moment one combination costs more than another. If you show RRPs to shoppers, a card carries one here too - the lowest RRP among the variations, printed beside the "From" figure exactly as it is on a product without options, so a grid of cards doesn't quote one on some listings and go quiet on the rest. Only RRPs that sit above what you're actually charging count, which is the same rule an ordinary product's follows.
...
The column has one row's worth of space, so it shows the first photo with a small **+2** (or +3, or however many) tucked in the corner to account for the rest. The × beside it clears the whole set for that row. Whichever photos you pick get filed into a **variations** folder inside the product's own folder in your media library, so all of one product's pictures live together without a hundred variation shots burying the three the listing itself uses. The picker opens in that variations folder too - though you can browse up and into any other folder, and the search box covers the whole library.

If your site has been running a while, your existing variation photos are still sitting in among the product's own. Nothing is wrong with that and nothing is broken; it's just untidy. Moving them is a one-off job for whoever looks after your site, run from their machine rather than from a button, because every picture it moves is a real move at your storage provider - point them at `modules/shop-variations/scripts/backfill-variation-folders.mts`, and ask them to run it *before* the small-copy tidy-up described in [Managing media](Managing-media#tidying-the-copies-of-a-library-that-predates-the-thumb-folder).
...
So each row of the grid has two tick boxes: **Image up front**, right beside the **Image** column, and **3D up front**, right beside the 3D model column - separate, because the combination with your best photo is not always the one with your best model, and you may only have one of the two to show off in the first place.

Tick **Image up front** and that combination's first photo joins the product's own gallery straight away. Want a different photo, or several? Open the row by clicking the combination's name and tick **Up front** under each photo you want shown - they go up front together, in the order they sit on the combination, and the tick box in the row comes on by itself (unpick the last one and it goes off again). A small **×2**, **×3** beside the tick box tells you at a glance which rows are showing more than one. It all joins the gallery, before anyone has touched an option, arriving behind the product's own pictures in the thumbnail strip. Tick it on three rows and all three turn up. From there it is an ordinary picture in the gallery: it appears on the product's **Images** tab alongside the product's own photographs, marked **Variation**, and you drag it wherever you like among them - see [below](#arranging-them-among-your-own-pictures). The same photos also come first among this product's variation pictures on its tile in a grid, ahead of the rest of the range, and a shopper pointing at that tile is shown one where the product hasn't a second photo of its own - so the finish you've chosen to show off is the one that answers the cursor, not whichever colour happens to be first in the grid.
...
So a promoted combination's photo turns up on the product's **Images** tab as a tile in the same grid as the product's own pictures, marked **Variation** with the combination's name underneath it. Drag it anywhere in that grid, or use its arrows. Whatever order you leave the grid in is the order the shopper gets: the thumbnail strip on the product page, the pictures behind the arrows on its tile in a grid, and the picture the page opens on if you drag one to the front. Point at that tile without clicking and it shows whatever you put second, so a finish dragged into second place is the one that answers the cursor. Press **Save** as you would for any other change to the product.
...
From version 0.1.128 the picture also stays put through the accessories box, if you have the Product Add-ons module. It used to let go at the end of the product's own choices, which is a shame, because picking a screen for a desk changes the picture just as much as picking the desk's finish does - and the customer was doing it with the photo already gone off the top.
...
**Colour & image previews** decides whether pointing at one of those swatches pops a bigger look at it - the full picture, or a proper block of colour rather than a 16px dot. It is on to begin with, and it is now set **per screen size**: the setting carries the same little desktop / tablet / phone switch the rest of the editor uses, so you can leave the bigger look on for desktop, where it costs a customer nothing to hover past it, and turn it off on phones. Worth doing, because a phone has no hovering: pressing a swatch to choose it *is* the hover, so the enlarged picture lands over the page whether the customer asked for it or not. Switch it off and the swatch simply gets on with being a swatch - on the Swatch only look the small name label still appears, since that is the only place the name lives. Leave a screen size blank and it follows the wider one, so setting desktop alone still sets all three, exactly as before.
...
**A sale hiding in one choice is pointed at.** Where only some of a product's variations are on offer - one width on clearance, say, while the rest are full price - a small red **Sale** badge lands on the choices that carry the reduction, next to their own names: "180cm **Sale**". The customer can see which one holds the offer instead of clicking through every combination hunting for it. In a drop-down, where a badge can't be drawn, it reads as a word beside the choice; a colour shown as a swatch with no name beside it wears the badge next to the swatch itself, so the offer is visible while they're looking rather than only once they point at it. It stays quiet where nothing is reduced, and equally where every choice in that row is - marking all four when all four are on offer says nothing the struck-through price beside the basket button hasn't already said. As the customer works down the choices it keeps up: settle on the full-price width and the badges disappear from the choices below it, because there's no longer an offer down that road.
...
**The basket button waits until they've finished.** It stays firmly off until every option has an answer - and rather than a vague "choose your options", it names the ones still outstanding: "Choose Width and Storage first". That wording is now written on the button itself, so the thing they're reaching for is the thing that tells them what it wants, instead of a line of small print underneath that a customer mid-tap never reads. A long list of outstanding options makes for a long sentence, so the button quietly sizes the wording to fit itself - shrinking it a little where that will do, and letting it run to a second line inside the button where it won't. Either way the whole message is there, on a phone as much as on a desk, and the button stays where it belongs beside the quantity box rather than barging onto a line of its own. Pointing at the button still shows the same words as a tooltip. The moment the last option is answered, a green line appears above the button reading back exactly what they've built - "Ready to add: 140cm · 2 Drawer Fixed Pedestal · Black" - so the thing they're about to pay for is written down in words rather than left implied by which buttons happen to be lit.
...
**And whether it can come back.** A third dashed **staff only** box says what the returns policy makes of the combination in hand - **Returns: accepted**, or **Returns: not accepted** followed by the exact sentence a customer would be given. Unlike the other two it speaks before you've finished choosing, on purpose: the answer is nearly always the product's own, so making you build a chair to find out something that was true of the whole range all along would be daft. Once a combination is settled its own answer takes over, and where that row genuinely differs from its product the box adds **this option only**. Customers see none of it, and it isn't sent to their browser.
...
### The link keeps up with the choices

As the customer picks their way through the options, the address in their browser quietly keeps up: each choice is written into the address as a readable little note of its own - `?seat-colour=oxford-blue&width=1600mm`, named after your options and their values. Copy the address at any point - fully chosen, half chosen, it doesn't matter - and whoever opens it lands on the page with exactly those choices already made: same options selected, same price showing, same photo up. **Reset options** clears the address back to the plain one along with everything else.

Sharing is rather the point. Paste the address into WhatsApp or anywhere else that shows link previews and the preview's picture is the chosen combination's own photograph - the same one the page opens on - not the general catalogue shot. A customer can send "this one, in this colour" and the message shows precisely that.

It plays nicely with the addresses that already existed. Every variation has always had a tidy link of its own (the one the basket uses), and those still work exactly as before - open one and every option is answered for you. Make a different choice on such a page and the address simply switches to the parent product's own address with your choices written on it, so what you copy always says what you're looking at. And the back button is left alone throughout: changing a colour never piles up history to click back through.

### Every combination in your sitemap

From **0.1.154**, each buyable combination is listed in your site's sitemap at its own address - the same readable one described above. So `?headrest=with-headrest&upholstery-colour=rivet-forge` is not merely a link a customer can share; it is a page search engines are told about and can send people straight to, arriving on the exact chair in the exact fabric.

A few things worth knowing before you go looking at the numbers:

- **Only whole combinations count.** A half-chosen address is the plain listing with a couple of controls pre-set, so it is left out. Switched-off variations are left out too, as are sold-out ones on a shop set to hide sold-out stock.
...
- **Without Stock Number** lists variations that track inventory but have never been given a count. A variation genuinely out of stock - its count set to nought - doesn't show up here; that's a real answer, not a gap. Variations that don't track inventory at all don't show up either, since they were never meant to carry a number.
...
- **Import / export** downloads every variant as a spreadsheet and lets you upload one back - one row per variant, with its options, SKU, **Sale SKU**, price, stock, **Min Qty**, **Returns**, barcode and weight. A blank Min Qty cell means the variant follows the product's own smallest order, and a 1 means the same thing, so leaving the column alone changes nothing. **Returns** works the same way: blank means the row follows the product, and only a plain `No` refuses. The extra price types travel too: **Sale Price, RRP, Trade Price and Cost Price** each get their own column beside the ordinary price, so a big change to any of them is a spreadsheet job rather than a variant-by-variant one. A blank price cell means the variant simply hasn't got that figure; fill it in and upload to set it. Handy for a big price change or setting up lots of variants at once. Create the parent products in the shop first; the spreadsheet fills in their options and variants. The download asks which columns you want first - everything is ticked to start with, and you can cut it down to, say, just the variant code and price. **Variant ID** always comes along and can't be unticked: it's how each row finds its way back to the right variant when you upload the file again. The option and value columns come as one tick, as do any extra columns other parts of the shop add. Your choice is remembered on that computer for next time.
...
- **A variation's name and web address follow the product's.** Both are built from the listing they belong to - "Impulse Cantilever Crescent Corner Office Desk - 160cm / Right Hand / Maple / Silver", at an address to match - and they now keep up when the listing is renamed or its address is rebuilt on the **Search** tab. Until now they were fixed at the moment the combinations were generated, so a product renamed afterwards had a basket full of lines quoting a name it hadn't gone by for months, at an address to match. Every variation already on your site has been put right, so the link in the basket is the one you'd expect. Long combinations are no longer chopped off halfway either, which is what used to leave two different sizes sharing an address with a "-2" stuck on the end.
