...
That does not mean no augmented reality - quite the opposite. You do not need to upload anything for it: the viewer builds the AR view itself, on the phone, from the model you already have. See [View in your room](#view-in-your-room-ar) below.
...
A shopper who would rather look at the photographs keeps them. Click a picture in the strip and the big view is theirs from then on: change a finish and they get that finish's first photograph, not the model put back over the top of it. The 3D thumbnail stays in the strip throughout, and one click on it hands the view back to the model - after which a change of finish leads with the model again, as before.

**Showing a variation's model before anyone has chosen.** Where a range's best model belongs to one particular variation rather than to the product itself, tick **3D up front** against that row on the **Variations** tab and its model joins the opening view, sitting behind the product's own. Handy where the product has no model of its own at all: promote the one variation that has, and the product page opens on something you can spin instead of a flat photograph. It stays put until every option has been answered and steps aside the moment one has, so it never ends up answering for a choice they didn't make - and equally never vanishes, or swaps itself for a different model, while the shopper is still working through the options. A neighbouring **Image up front** box does the same job for that variation's photograph - tick either, both, or neither, since the combination worth leading with in 3D is not always the one with your best picture. The whole rule is written up under [Shop variations](Shop-variations#showing-off-a-finish-before-anyone-has-chosen).
...
**If you swap out or add to a product's 3D files.** Attach a new model, or a replacement of the same file, and the shop measures it as it draws it - the weave comes out at its proper size straight away, with no panel to open and no button to press. A file's size and how its finish is mapped are both things the viewer can see for itself, so it reads them off the model rather than waiting to be told.

This matters most for a file that is not quite like its neighbours. Where one model in a range comes back from the supplier mapped at a different scale to the rest - easily done, and invisible until it is on screen - it is now measured on its own terms and drawn to match, instead of borrowing a figure taken from a different file.

Opening the material configurator still re-measures everything and saves it, and you will still see a note saying it has; that remains worth doing so the saved setup matches what is attached. But it is no longer the difference between a finish at true size and one that is not. The two measurements that stay yours to fill in are the ones no model can state about itself: each swatch's **size**, and each variation's **overall height or width**. Where either is missing, that surface still shows in the right colour and only its scale sits neutral, exactly as before.

**Where an attribute is the same on every variation.** Not every finish changes from one variation to the next. The legs on a desk range, or the frame on a chair, are often one finish across the lot - and on the **Product attributes** screen that sort of attribute is set once on the product itself rather than variation by variation. Point a material part at one of those and it paints exactly as it would a per-variation attribute: every variation shows the finish set on the product. The same goes for the overall height or width you scale by, which on most ranges is one measurement for the whole product. Where a variation does carry a value of its own for that attribute, that one wins - so a range with one leg finish throughout, bar the one variation that comes in black, needs nothing more than setting black on that variation.

Because a finish set that way is known before the shopper has chosen anything, it shows on the product's opening 3D view too, rather than waiting for a full combination to be picked. The parts that do depend on their choices stay as the model file has them until they choose.
...
**Painting the accessory in a combined view.** Where a product is shown with one of its add-ons fitted - a desk drawn with the pedestal that goes under it - the combined model carries the accessory's parts as well as the product's, and the shopper picks that accessory's finish on the accessory, not on the desk. So the **Colour from** box lists a third group, **Add-on options**, holding the choosers belonging to every accessory the product can be bought with, each one named after the accessory it came from. Point the pedestal's carcass at the pedestal's own Finish and it paints itself as the shopper picks; do the same for its handles and they follow suit. The list follows the chain, so an accessory that has accessories of its own - a set of handles for the pedestal - is offered too. A product with no add-ons, or a shop without the add-ons feature, sees no such group and nothing changes.

Two things worth knowing. A part is only painted where the accessory is actually ticked, so an untouched accessory leaves its parts exactly as the model file has them. And a colour with no swatch picture behind it paints nothing at all, quietly, so any finish you expect to see on the model needs its picture setting on the **Product attributes** or **Variations** screen first.
...
**It only shows where it works.** The button appears on phones and tablets that support AR, and nowhere else - so there is nothing on a desktop, where a shopper sees the ordinary viewer exactly as before. If the model turns on screen, it can go in the room: the AR view is built from what the viewer is already drawing, so it works the same whatever format your model arrived in. There is no "sorry, not supported" message to nobody; the button simply isn't there unless it will do something.

**Sizing mostly looks after itself.** On products that use the material configurator, the real measurement you already gave it - the overall height or width that scales the fabric - is reused to stand the product on the floor at its true size. A boardroom table you told the shop is 2.4m wide arrives 2.4m wide, with nothing more to set. For plain 3D products that have no such measurement, it falls back to a single site-wide **Real-world size** (the longest side in metres, about 1 suits most furniture) under [Fine-tuning the viewer](#fine-tuning-the-viewer) - a starting point the shopper can pinch to adjust. Either way, if you would rather not offer AR at all, there is a switch to turn it off for the whole site.
...
**View in your room (AR).** Whether the **View in your room** button is offered, and the fallback size for products the shop has no real measurement for. It is on by default and shows only where a phone can actually use it (see [View in your room](#view-in-your-room-ar)). Products set up with the material configurator already stand at their true size automatically, so **Real-world size** here only applies to plain 3D products: it is the longest side in metres when such a product first appears on the floor - around 1 suits most furniture, and the shopper can pinch to adjust from there. Turn the whole thing off here if you would rather not offer it.
...
**The "View in your room" button isn't showing.** It appears only where it can work, which rules out most of the places you might be looking. On a desktop or laptop there is no AR to offer, so it never shows there - test it on an actual phone or tablet. On a phone it needs a device new enough to support AR - most of the last few years are - and the model has to be one the viewer can actually draw (if it turns on screen, it qualifies, whatever the format). Also check it hasn't been switched off under **Settings → Shop → 3D Viewer**. If all of that is in order and it still doesn't appear, the phone or its browser simply doesn't support AR, and there is nothing to be done at your end.
