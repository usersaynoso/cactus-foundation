...
**Showing a variation's model before anyone has chosen.** Where a range's best model belongs to one particular variation rather than to the product itself, tick **3D up front** against that row on the **Variations** tab and its model joins the opening view, sitting behind the product's own. Handy where the product has no model of its own at all: promote the one variation that has, and the product page opens on something you can spin instead of a flat photograph. It leaves the view as soon as a pick rules that variation out, so it never ends up answering for a choice they didn't make - a promoted model the shopper's choices still allow stays where it is. And where the product has no model of its own and nothing promoted survives the picks, the variations still on offer stand in for it rather than leaving a blank space, which is what a product whose 3D lives entirely on its variations used to get halfway through choosing. A neighbouring **Image up front** box does the same job for that variation's photograph - tick either, both, or neither, since the combination worth leading with in 3D is not always the one with your best picture. The whole rule is written up under [Shop variations](Shop-variations#showing-off-a-finish-before-anyone-has-chosen).
