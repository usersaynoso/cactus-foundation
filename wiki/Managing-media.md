...
Press **Run check**. It can take a little while on a big library, because it genuinely reads through everything your provider holds. Then it tells you about four things:
...
- **In use, but with no entry here** - files your site is genuinely serving that never got an entry in the library. A 3D model or a product photograph added in bulk can land this way. They look like leftovers and are anything but, so they're listed for information only, with no button anywhere near them. Nothing here is ever offered for deletion.
