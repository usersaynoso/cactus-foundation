...
In the media library the Upload button spells out where files will land (**+ Upload to Photos**, say), and as soon as you pick or drop files the **notification bell** pops open with a live progress list, showing each one filling up as it goes and ticking green when it's done. Files go up **three at a time** rather than in single file, so a few hundred photos take minutes rather than an afternoon, and one slow-moving file doesn't hold up the queue behind it (three rather than more, because some browsers - Safari in particular - flatly refuse anything past a modest number of simultaneous connections). If one won't do - wrong sort of file, or a stray oversized SVG - it's flagged there and then with the reason, so nothing fails in silence. And when an upload does go wrong mid-flight, the message now says **which step failed and why** - the check for existing names, the hand-off to storage, or the final filing - rather than a bare "upload failed", so there's something worth passing on if you need help. These upload entries sit above your usual notifications and don't count as unread - clear them with a click once you're done.
...
**Renaming a product takes its whole folder with it.** Not just the product's own photographs: its variations' pictures, its 3D models, its spec sheets, colour swatches and description videos all move to the folder named after the new name, in one go. The folder itself moves the moment you save, so the library shows it in the right place straight away; the files are then copied across behind the scenes. A big range can have several hundred of them, so on a very large product the copying may not all fit into one save - nothing is broken while it catches up, every file keeps showing from where it was, and your next save of that product (or the tidy-up below) finishes the job. Two folders that turn out to belong to the same product are merged rather than left side by side, and a file that would land on a name already taken is kept under a slightly different one, never written over.

(Before this, renaming a product moved its own photographs and left everything else under the old name. If you renamed products back then, the tidy-up below finds those leftovers and puts them where they belong.)

There is one thing that does *not* re-file itself, and it is worth knowing about: **renaming a category, or moving it under a different parent.** The files already filed under the old name stay where they are, which leaves the same category showing up twice in your library with its pictures split between the two spellings. The same goes for a product renamed or re-categorised by a spreadsheet import, which changes the product without touching its files.

So the shop tells you when it has happened. Open **Shop → Categories**, or rename or re-parent a category there, and if anything is filed under a name the shop no longer uses, a note appears at the top of that screen saying how many files are affected, with a **Tidy up filing** button. Nothing is broken while you wait - your shop shows every one of those pictures either way - so you can do it there and then or leave it for later. Pressing the button moves them across in small batches and counts them off as it goes (a product with more files than one batch has time for simply goes round again), and every place that pointed at a file follows it automatically: product galleries, variation swatches, 3D models, spec sheets, description videos, even old saved quotes.

A picture that two products share is left in whichever of their folders it already sits in. Moving it into one product's folder would only take it out of the other's, and the note would never go away.
...
Four of them you can put right on the spot. **Correct these** rewrites the wrong sizes to match the real files, and changes nothing else. **Delete all leftovers** removes the stray files from storage and tells you how much room you got back. That one asks you to confirm first, and only appears if you're allowed to delete media. Before it offers you anything, the check reads through every page, every layout and every add-on's own records to see what your site is actually using, so a file in use can't reach that list. If any of that can't be read for whatever reason, the check reports no leftovers at all rather than guess. It is not undoable from here.

**Add these to the library** gives an entry to the files your site is using that the library never knew about. Nothing is moved, copied or re-saved - each file stays exactly where it is, and simply gains the entry it should have had, filed in the folder its own address already puts it in. Afterwards they turn up in the library like anything else, count towards your storage total, and can be renamed, tagged or replaced in the usual way. It's the only way that list ever empties, and it needs permission to upload media. A file whose name doesn't say what kind of file it is gets left alone and named for you, rather than being guessed at.
...
Big clean-ups - hundreds of entries, leftover files or additions at once - are worked through in batches, with a running count shown as it goes. If a batch stumbles along the way it's quietly retried a couple of times before the check admits defeat, so a wobbly connection doesn't send you back to square one.
...
- **Models that arrived already squashed now go through too.** Most design tools offer to squash a model on the way out, and plenty of the free "make my model smaller" websites do it whether you ask or not - so a fair few models turn up having already had a go at this. For a while those were the one kind Cactus couldn't finish: it would unpack the model perfectly well and then fall over putting it back together, which showed up as a grumble from the **Optimise** button, or as nothing at all on upload, the file quietly staying at full weight. Rather a poor showing, given they were often the very models most worth slimming. Fixed. If you have models sitting there still wearing the **Optimise** button after a failed go, press it again and they'll go through. One thing to expect: a model that was already squashed to within an inch of its life may come back saying it's as small as it gets, which is Cactus being honest rather than idle - the file is left exactly as it was.
- **Striped desks get straightened out on the way in.** Furniture models very often come out of a catalogue tool that shades the whole catalogue from one tiny shared swatch, a few pixels across, with every part of every product told to read one particular pixel of it. It is a neat trick and it works perfectly - right up until a part is re-cut so a shopper's finish can tile across it at a real size, at which point that reading wanders across the swatch and the surface picks up broad bands of the wrong sheen. The picture is right, the wood grain is right, and the top looks like it has been polished in stripes. Cactus now spots that as a model arrives, works out the single shade the part was standing on, and pins it there instead. Models that use the trick properly - which is nearly all of them - are not touched at all.
