...
The clever bit is what you can do next. Open a video and choose **Convert to scroll sequence**, and Cactus quietly rebuilds it into a *scroll sequence*: a smooth, frame-by-frame animation with the plain studio background - and the drop shadow that usually tags along with it - removed, so it looks right on both light and dark pages. See-through parts of a product (a mesh chair back, say) keep their see-through-ness rather than carrying a ghost of the white studio wall around with them. You choose where it's stored and give it a name; every conversion runs at the one quality set under **Media › Scroll sequences** (high quality by default - 30 frames a second, up to 1920px wide).
