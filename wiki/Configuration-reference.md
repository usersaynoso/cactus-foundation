...
| `SEQUENCE_WORKER_URL`, `SEQUENCE_WORKER_SECRET` | No | The video conversion service that turns a product video into a scroll sequence (see [Managing media](Managing-media)). The URL is the service's address; the secret is the shared key that authenticates both directions. Unset simply hides the **Convert to scroll sequence** action. |
| `SEQUENCE_FLY_TOKEN` | No | Fallback Fly.io API key for scroll-sequence conversions. With a key (here or saved under Media → Scroll sequences, which wins), each conversion runs on its own short-lived machine - as many videos convert at once as you start, and every machine is removed when its job finishes. Without one, conversions queue on the single service. |
