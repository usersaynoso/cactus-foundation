...
| `SPACE_PLANNER_RENDER_URL`, `SPACE_PLANNER_RENDER_SECRET` | No | Space Planner module - enables photoreal pictures of a finished room (see [Space Planner](Space-planner)). The URL is where render jobs are sent; the secret is what proves the request came from your site. Both must be set, and a machine has to be listening at the other end. Everything else in the planner - the plan, the item list, printing and quotes - works perfectly well without them, and the Pictures screen says plainly when they are missing. |
