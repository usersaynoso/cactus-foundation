...
Every email your site sends: the member ones (welcome, verification, magic sign-in link, digests, security alerts), the account and security ones (login code, password changed, account recovery), and one section per module you have installed - Shop's order emails, Boards digests, contact form notifications, and so on. Only modules you actually have installed appear.

The tab opens on a single **Section** dropdown and nothing else. A busy site can send well over a hundred different emails, and all of them at once is a wall rather than a menu - so pick the section you are after first. The emails in it then appear as a row of pills across the top; the ones you have reworded are marked **Edited**, and any you have switched off are marked **Off**. If your site only has the one section, it is chosen for you.

Pick an email from the pills and the editor opens underneath, with the finished email alongside it on the right:
...
- **The wording you can drop in**, listed as chips. `{{siteName}}`, `{{orderNumber}}`, and so on. Each one has a small copy button beside it, so you can drop it into the message without retyping the curly brackets and wondering later why nothing filled in. Anything marked with a `*` has to stay in: a sign-in email with the code taken out is no use to whoever receives it, so saving one is refused rather than allowed and regretted.
...
- **The preview sits permanently to the right** and redraws itself a moment after you stop typing - there is no button to press and nothing to close afterwards. It shows the finished email, wrapper and all. Anything that only exists at the moment of sending - an order number, a sign-in code, a link - appears as an obvious stand-in so nobody mistakes a preview for the real thing. Things the site already knows about itself, `{{siteName}}` chief among them, show their real values, because seeing your own site's name replaced by a placeholder is a fine way to conclude the tag is broken and type the name in by hand. On a narrow screen the preview drops underneath the editor rather than squeezing in beside it. Optional lines - the ones you wrap in `{{#if ...}}` so they disappear when there is nothing to say - are shown switched **on**, so the preview is the fullest version of the email rather than the barest one.
...
| Hide "Reject all" until the visitor opens "Manage preferences" | The banner first shows only **Accept all** and **Manage preferences**; **Reject all** appears alongside the category switches once they are opened. Reopening the banner from a Cookie settings link goes straight to the switches, so it shows there. Changing this does not re-prompt anyone. UK and EU regulators expect rejecting to be as easy as accepting, so take your own advice before switching it on. | Off |
| Dismiss button label | Button label used when the banner is notice-only (no optional categories configured) | `Got it` |
| Cookie categories | The list of cookie categories visitors can accept or reject, in the order visitors see them. The **Necessary** category is always present, always on, pinned to the top, and cannot be removed. | Necessary, Preferences, Analytics, Marketing |
...
**Category order:** The arrows beside each row move a category up or down, and that order is exactly what visitors get - in the banner's **Manage preferences** panel and in the panel on the privacy policy page alike. **Necessary** stays at the top: its own arrows are greyed out, and nothing can be moved above it. Reordering is presentation only, so it never re-prompts anyone who has already decided.

**Category changes:** Adding or removing a category, or changing whether a category is required or on by default, automatically triggers re-consent for returning visitors. Purely cosmetic changes (renaming a label, editing copy, reordering the list) do not.
...
| `GOOGLE_PLACES_API_KEY` | No | Address Lookup module - fallback Google API key, used when the module is set to look addresses up with Google rather than Ideal Postcodes (see [Address Lookup](Address-Lookup)). The key saved on Shop → Settings → Address lookup takes precedence. Needs the Places API switched on in the Google Cloud console. |
| `GOOGLE_ADS_CLIENT_ID`, `GOOGLE_ADS_CLIENT_SECRET`, `GOOGLE_ADS_REFRESH_TOKEN`, `GOOGLE_ADS_CUSTOMER_ID` | No | Google Shopping module - the Google Ads connection, which is a **different account and a different sign-in** from the Merchant Center service-account key (see [Google Shopping](Google-Shopping#google-ads)). All four must be set before the site can read what your adverts cost or tell Google Ads which clicks turned into a sale; the two jobs are then switched on separately, and both start off. Set them on Shop → Settings → Google Shopping, under Google Ads (or in `.env.local` locally). The account number is the ten-digit one at the top right of Google Ads; dashes are stripped. Credentials are stored with the hosting project, so a newly pasted value needs a deployment before it works. |
| `GOOGLE_ADS_LOGIN_CUSTOMER_ID` | No | Google Shopping module - the **manager** account number, needed only when the Google Ads account above sits underneath one. Leave it unset otherwise: Google's own guidance is to omit the header rather than repeat the account number into it. |
| `GOOGLE_ADS_DEVELOPER_TOKEN` | No | Google Shopping module - **no longer needed.** Google retired developer tokens on 9 September 2026; the header is now ignored, and what a connection is allowed to do is decided by the Google Cloud project that owns the sign-in above. An install that already carries one keeps sending it and comes to no harm, and nothing is gated on it - a site with no token is not held back. |
...
| `CACTUS_PRUNE_BUILD_CACHE` | No | Automatic on Vercel. Set to `0` to disable build-cache cleanup, or `1` to explicitly enable it locally. Cleanup keeps dependencies when they fit, compresses compilation work when needed, and only removes unneeded packages to meet the budget. Previously packed compilation work still restores when cleanup is switched off. |
| `CACTUS_BUILD_CACHE_BUDGET_MB` | No | Positive, finite size in MB for the build-cache cleanup's on-disk budget. Defaults to `1400`. This is a safety allowance, not a change to Vercel's own cache limit; raising it may cause Vercel to discard the whole cache. |
