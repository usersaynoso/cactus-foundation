...
| **Phone** | A click-to-call phone number. Type the number in and it becomes a proper `tel:` link, so tapping it on a phone starts a call. Type it however reads best - spaces and brackets and all - and Cactus tidies it up behind the scenes for the dialler. Add **Display text** if you'd rather show "Call us" than the raw number. A **Phone icon** can sit alongside it, and you can have it appear on phones only, where tapping to call actually makes sense. Pick the font, the colour and a separate **Hover colour**, and switch on **Underline number** to underline it - the underline's colour and thickness settings only appear once you do. There's a hover underline option too, for the number that only underlines when someone points at it. |
