...
| **Text** | A paragraph of body text. **Text size** is a menu: the three named sizes it always had (Base, Lead, Large), and under them a run of plain pixel sizes from 8px up to 64px, for when the size wanted is a size rather than a step on a scale. Per screen size, like the alignment beside it. |
| **Rich Text** | A full text editor with bold, italic, bullet lists, numbered lists, links, and blockquotes. A **Text colour** setting recolours the whole block - pick one of your site colours or set a custom colour of your own. Links keep their usual link colour so they still look clickable. **Text size** sets the body text in pixels; leave it on Default and nothing moves. Headings inside the block keep their own sizes. |
