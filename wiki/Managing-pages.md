...
| **Split** | A two-column layout where each side is a separate drop zone. Good for side-by-side content. Pick how the width is shared - an even 50/50, a lopsided 70/30, and so on - or choose **Auto**, where the left column takes only the width its contents actually need and the right column helps itself to the rest. Auto suits a fixed-size thing on the left, like a photo; give it a paragraph of text and it will simply take the full share, since text will happily be any width you like. |
