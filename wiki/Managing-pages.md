...
**Every colour also has a dark-mode override.** Once you've chosen a colour - a text colour, a border, an icon, a background, any of them - a **Dark mode colour** picker appears just beneath it. Set one to give that colour a different look for visitors browsing in dark mode; leave it blank and dark mode simply reuses your light colour, so nothing changes unless you ask it to. It's the same idea as the dark-mode overrides in the Styles editor, just applied to a single block. (Your palette swatches already carry their own dark variants, so this is mainly for custom colours, or when you want a particular block to swap to a different swatch in the dark.)
...
That breathing gap is 24px down each side, and every width except **Edge to edge** has always had it whether you wanted it or not - which was a nuisance if you'd set a **Custom max-width** of, say, 1920px and then found your content stopping 24px short at both ends. The Section's **Side padding** setting is the answer: leave it blank and you get the automatic behaviour described above, exactly as before, or type a number and that's the gap you get. Set it to 0 and your chosen width is the width you get, right to the last pixel. Per screen size, naturally, so you can claim every pixel on a big monitor and still keep your text off the edges on a phone.

**Background patterns, and a different one for dark mode.** The **Section**, **CTA Banner** and **Hero** blocks all take a tiling background pattern: choose an image or SVG from your media library under **Background pattern** and it repeats across the block, sitting over whatever colour, gradient or photo is already there rather than replacing it. **Pattern size** decides how big a single tile is - blank leaves the image at its natural size - and, like the spacing settings, it can be set separately for desktop, tablet and mobile, which is usually the difference between a tasteful texture and wallpaper. Because a pattern drawn for a pale background tends to vanish on a dark one, there is a second picker, **Background pattern in dark mode**; fill it in and dark-mode visitors get that image instead. Leave it blank and everyone sees the same pattern, which is fine for anything low-contrast. There is a matching **Pattern size in dark mode** as well, since the same tile often wants to be larger to read against a dark background - it takes desktop, tablet and mobile sizes like the light one, and left blank it simply uses whatever the light size is. All of these stay hidden until you have actually picked a pattern.

**Faint lines across a pattern sort themselves out now.** They were never in your artwork. A pattern's size sets how *wide* one tile is and the browser works the height out from the picture's own proportions - and for any tile that is not square that height is almost always a fraction of a pixel. A screen cannot draw a fraction of a pixel, so the bottom row of every tile came out pale and you got a faint line across the pattern once per tile, on top of whatever lines the picture itself contains. Square tiles never did it, which is why some of your patterns were always fine.

Cactus now measures the picture when you pick it and quietly nudges the size at display time to the nearest one that comes out whole - ask for 96px on a 660x472 tile and it draws at 82.5px, which is 59px tall exactly. The nudge is a couple of pixels at most and you are unlikely to notice it. The number in the size box stays exactly as you typed it - the nudge happens on the way to the screen, never in your settings - and sizes typed in rem are nudged the same way. As a second line of defence, a pattern whose picture is fully solid also gets its own base colour painted underneath it, so even on a zoomed page or an unusually scaled screen - where no amount of tidy arithmetic can keep every edge on a whole dot - anything peeping through a join is the pattern's own colour rather than whatever sits behind it. Patterns with see-through parts are left alone, since showing what is behind them is their whole job.


The **whole page** can carry one as well. Click away from every block to reach the page's own settings, next to **Page background**, and the same three settings are there: the pattern tiles behind everything on the page, underneath any Section that paints a background of its own. The one difference is that these three stay listed whether or not you have picked a pattern - the page's settings have no way of hiding a box the way a block's do. Your email wrappers take a pattern too, with a couple of email-shaped caveats: see **Settings → Email → Email wrapper designs**.
...
| Block | What it does |
|-------|--------------|
| **Section** | A full-width container with a background colour, image, or gradient. Drop other blocks inside it. Great for creating distinct visual sections. When the background is a colour, a **Colour opacity** slider lets you make it see-through - handy for a soft, readable panel sitting over a background photo - and a **Dark mode colour** picker lets you choose a different colour for dark mode (so a panel can be light in light mode and dark in dark mode). Both are available on the Hero block too. The **Vertical padding** setting includes a **Full view height** option that makes the section fill the whole screen - useful for a landing section a visitor sees in full before scrolling. **Content vertical alignment** (top, middle or bottom) decides where the content sits when the section is taller than the content inside it - pair it with Full view height to centre a hero on the screen. It can be set separately for desktop, tablet and mobile. The **Border** dropdown only reveals its colour and width settings once you actually choose a border style; set it to None and they stay out of your way. A **Background pattern** sits on top of all that: pick a tiling image or SVG from your media library and it repeats across the whole section, over the colour or photo underneath. **Pattern size** sets how wide one tile is (leave it blank for the image's own size), and can differ per screen size, so a pattern that looks right on a computer needn't be a coarse mess on a phone. The size is nudged to the nearest one that tiles cleanly - see the note above about faint lines. **Background pattern in dark mode** takes a second image for visitors in dark mode - a fleck that reads beautifully on cream is usually invisible on charcoal - and left blank, the same pattern is used in both. |
| **Grid** | A two-to-four column grid. Drop content into each column independently. **Stack columns** decides when the columns pile up underneath each other on smaller screens: **On mobile** (the usual - side by side on computers and tablets, stacked on phones), **On tablet and mobile** (stacked from tablet width down), or **Never** (stay side by side at every size). It works even when you've set your own **Column widths** to Manual. Speaking of which: the per-column width boxes only appear while Column widths is set to **Manual**, and switching back to **Equal** (or any preset) drops those custom widths straight away. **Vertical align** moves every column together (top, middle, bottom or stretch); underneath it, **Col 1 / 2 / 3 / 4 vertical align** does one column on its own and leaves the rest where they are, so a page number can sit on the bottom line while the address beside it stays at the top. Each column is set to **Follow the grid** until you say otherwise. |
| **Split** (retired) | The old two-column block. It no longer appears in the block list, and it cannot be dropped inside a Section, Grid, Group or member gate - inside one of those its two columns published as empty space, however right it looked while you were editing. Splits already sitting directly on a page or layout carry on working and can still be edited, so nothing you have built needs changing - including a setting called **Order once stacked on a phone**, which decides which of the two columns goes on top when they stack: the left one (as always) or the right one, handy when the left column is a long summary a phone visitor would rather scroll past than through. For anything new, use **Grid** with two columns: the same width choices are on its **Column widths** setting, including 70 / 30, and it works anywhere. |
...
| **Divider** | A horizontal line to visually separate content. Its colour comes from your site's swatches, or pick any custom colour. **Space above** and **Space below** set how much air it leaves, from none to extra large, and there is an exact box under each if you want a particular number of pixels - useful on a document footer, where a rule is being lined up against small print. Left alone it keeps the roomy gap it always had - except on the Document Footer layout, where every block starts closed up because the whole strip is only a few millimetres tall. |
...
| **Text** | A paragraph of body text. **Space above** and **Space below** decide how much room it leaves around itself, with an exact box under each - the Padding setting beside them has only ever been left-and-right. **Text size** is a menu: the three named sizes it always had (Base, Lead, Large), and under them a run of plain pixel sizes from 8px up to 64px, for when the size wanted is a size rather than a step on a scale. Per screen size, like the alignment beside it. |
| **Rich Text** | A full text editor with bold, italic, bullet lists, numbered lists, links, and blockquotes. A **Text colour** setting recolours the whole block - pick one of your site colours or set a custom colour of your own. Links keep their usual link colour so they still look clickable. **Text size** sets the body text in pixels; leave it on Default and nothing moves. Headings inside the block keep their own sizes. **Space above** and **Space below** set the room around the whole block, and **Space between paragraphs** closes up (or opens out) the gap between the paragraphs inside it - handy where a two-line address is meant to read as one address rather than two. |
...
| **CTA Banner** | A call-to-action banner with a heading, description, and button. **Vertical padding** sets how tall the banner is - how much breathing room sits above and below the text - and can differ per screen size. Left at None, the coloured panel hugs the text, which is how it has always behaved. It takes the same background pattern settings as the Section, so a banner can carry a subtle tiling texture over its colour. **Text colour**, **Link colour** and **Link hover colour** are yours to set on any of the four backgrounds, not just Custom - each with its own dark mode alternative, so a banner can read one way by day and another by night. Only the **Background colour** picker is tied to choosing **Custom colours**, since the three presets bring a fill of their own. |
| **Icon Link** | A lone icon that links somewhere - made for header icon rows, where it sits happily next to the theme toggle, sign-in and basket icons and matches their colouring without any fiddling. Pick one of the built-in outline icons, or choose **Uploaded image** and pick an SVG or PNG from your media library (the picker's upload button takes SVGs now, too). An uploaded image is recoloured to your chosen icon colour by default so it behaves like the built-in set; switch **Icon image colouring** to keep the image's own colours instead. Set the **Link URL**, whether it opens in a new tab, and a **Title** - shown as a small tooltip under the icon when someone hovers, and read out to screen readers. **Icon colour** and **Hover colour** each take a light and a dark mode pick from your site palette (or any custom colour); left blank, they use the same colours as the theme toggle beside them. **Who can see this** can restrict the icon to signed-in admins - handy for trying a new link on the live site before showing customers. |
...
| **Feature video** | Plays one of your own videos beside a headline and a paragraph - the sort of thing that shows a product doing something rather than sitting still. Pick the video from your [media library](Managing-media#videos), write a **Title** and **Text**, and use **Title and text position** to put the words above the video or on either side of it. Side-by-side neatly becomes words-above-video on phones. It plays silently by default, on its own, once it scrolls into view, and pauses again when it leaves, so nothing is downloaded until a visitor is nearly looking at it. **Corner radius** starts at `16px`, which matches a shop product's main photo, and **Framed like a product image** puts the same hairline border and soft backdrop behind it - worth keeping on for studio clips shot on white, which would otherwise glare on a dark page. Turn on **Show player controls** if visitors should be able to pause or scrub. |
...
| Block | What it does |
|-------|--------------|
| **Hero** | A large banner section with a heading, sub-heading, and an optional button. Great for the top of a homepage. It takes the same background pattern settings as the Section and CTA Banner, so a tiling texture can sit over the hero's gradient, colour or photo. |
