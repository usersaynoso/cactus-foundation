...
- **It only runs, and bills, while it is needed.** Fly suspends the machine once nothing is connected to it, and wakes it the moment something is: a visitor opening the chat, a staff member opening the console or the inbox, the Online/Offline switch, the nightly backup. Ordinary page views never touch it - whether anyone is on is kept in the site's own database (written by the Online/Offline switch) and read from there. Staff tabs only hold a live connection while the console is open or the inbox page is on screen; a closed console keeps its unread badge current from the site's own copy of the chats instead. The *Chat server* card under Settings → Live Chat reads the machine's state from Fly without disturbing it, so it shows *asleep* when it is; the chat backups list on Settings → Backup does wake it, as it has to ask the machine what it holds.
...
- **When the switch says Offline, the widget says so too.** The bubble changes to *Leave us a message*, and the panel opens on its front screen rather than straight into the composer, so the visitor is told nobody is on before they start typing. What it says there is yours: Settings → Live Chat → Widget → *What to say when you are away*. Say what happens to a message left out of hours - people will sit waiting for a reply otherwise. The same sentence is the bubble's tooltip while you are away.
  - The site counts you as on if *any* agent is online. It learns this when someone flips the switch in Cactus, and re-checks whenever the Cactus inbox is opened. Flip availability in the Chatwoot mobile app instead and the site catches up next time somebody opens the Cactus inbox - use the Cactus switch if you want the bubble to follow at once.
  - The chat server used to print *Typically replies in a few minutes* on that screen whatever the switch said, because it was quietly reading a set of Monday-to-Friday office hours nobody had asked for. The module now clears those the first time the inbox is opened after an update, and the line reads honestly from then on.
...
- **A chat button in the phone bottom bar.** If your header or footer carries a [Mobile Bar](Appearance-and-design#a-bottom-bar-for-phones), one of the cells you can put in it is *Live chat: message us* - a message icon with your own caption under it. Pressing it opens the chat exactly as the bubble does, cookie question, invisible challenge and all, because it asks the widget to open rather than trying to start anything itself. The Live Chat block still has to be on the page for there to be a chat to open. The bubble carries on as normal alongside it - it is what starts the chat, after all - and lifts itself clear of the bar rather than sitting behind it. Pair the cell with *Hide the label on phones* and you get a tidy icon bubble above a tidy bar rather than two competing pills. Staff get the same button: pressing it opens their agent console rather than a customer chat, so a chat can still be answered on a phone with the console hidden.
- **An unread count on the bar's chat button.** When a reply lands while the visitor has the chat shut, the message icon in the bottom bar picks up a number, in the same badge colours as the basket count. It comes from the chat's own tally of replies they have not seen, stays put as they move between pages, and clears the moment they open the chat to read it. For staff it shows the same number as their console button - chats waiting on an answer. It keeps counting as they browse: once a visitor has chatted, later pages that visit reconnect the chat quietly behind the bubble, without opening it, so a reply sent while they look around still arrives - as the badge, and as Chatwoot's own little preview of the message. Someone who has never opened the chat still loads nothing at all.
- **The bubble makes way for the slide-out basket.** On a shop whose basket slides out over the page, the bubble used to sit on top of the panel's **View full basket** button. While the basket is open the bubble now disappears and the panel carries the chat instead: a full-width button under **View full basket**, in the bubble's colours and with its wording (*Leave us a message* when you are away). Pressing it shuts the basket and opens the chat, cookie question and all. Shut the basket and the bubble is back. Nothing to set up; with chat switched off there is no button, and a site with no shop never notices any of this.
- **The phone bar's chat button asks about cookies too.** Pressing the Mobile Bar's chat cell (or the basket's chat button) before the live chat cookie has been allowed now shows the same *Live chat needs a cookie* notice the bubble does, rather than starting the chat regardless.
- **The bubble can go entirely on phones.** Settings → Live Chat → Widget → *Hide the bubble on phones*. For a site whose bottom bar already carries a chat cell, the bubble is a second door to the same room sat over the page on the smallest screen there is - this shuts it, and the bar cell becomes the way in. It applies to staff too: the floating agent console hides on phones alongside it, and the bar cell opens that instead. Desktop keeps its bubble and its console. Off by default, and worth leaving off unless something else on the phone opens the chat, or there is no way in at all.
...
- **On the public site**: staff with the `livechat.view` permission see a floating agent console instead of the customer widget - answer without leaving the page. It's styled to match the customer widget (brand header with the Online/Offline switch, avatars, chat bubbles), follows the site's dark mode, and while closed it still announces new messages within about half a minute - the button pulses red with an unread count. (It used to hold a live line open to the chat server for this, which kept the server awake and billed around the clock for as long as anyone had the shop open; the live line now opens with the console.) It clears the phone bottom bar rather than sitting behind it, and hides on phones when *Hide the bubble on phones* is on - the bar's chat cell opens it in that case.
...
- **Knowing when to press it**: the status card compares Cactus *builds* of the chat image, not just Chatwoot version numbers. Every build is tagged like `v4.16.2-cactus.4`, so a Cactus-only change to the chat (a tweak to how the widget looks, say) shows as *update available* even though Chatwoot itself has not moved. The button moves the machine to that exact build rather than to `latest`, so the card can always say which one is running. A server still on `latest` from before this is matched by what it actually pulled; if that cannot be worked out, the card says so and one press sorts it. A newer Chatwoot release that has not been built into the image yet gets its own line, so you know it is coming rather than wondering why the button does nothing new. The build list comes straight from the public image registry, checked at most every ten minutes, and costs no GitHub allowance.
