...
- `collectModulePublicHead(siteUrl)` (generated into `lib/modules/public-head.ts`, re-exported here) - calls `getPublicHead(siteUrl)` from each module's `lib/head.ts` (if present), swallowing per-module errors, and returns `{ jsonLd, meta }`. `app/(public)/layout.tsx` renders each JSON-LD object as a `<script type="application/ld+json">` and each meta entry as a `<meta>`, so a module can contribute to **every** public page - core pages and other modules' pages alike - rather than only to pages it serves itself (core 0.5.1418). Its live consumer is `ultimate-seo`, which emits the site-wide Organization/WebSite structured data and the `twitter:site` tag from its saved profile. JSON-LD is deliberately rendered in the body rather than the head: search engines read it from either, and a layout has no way to contribute to `<head>` per request.
