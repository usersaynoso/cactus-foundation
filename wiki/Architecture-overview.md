...
The site registers exactly **one** cron job with Vercel: `/api/cron/dispatch`, hourly, declared in a committed `vercel.json` at the repository root. Everything else core or a module wants scheduled hangs off that single tick.
...
The practical cost is that a schedule is honoured to the tick rather than to the minute - `30 6 * * *` runs at the 07:00 tick on a paid plan, and once a day on Hobby, which is all Hobby ever allowed.
