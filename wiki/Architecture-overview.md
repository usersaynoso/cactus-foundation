...
The header has to be set from the proxy because a Server Component cannot set response headers and the route segment config that can (`revalidate`) is a build-time constant, which could never be driven by a switch in the database. It works because Next.js writes its own `Cache-Control` only when one is not already present (`sendRenderResult`, `next/dist/server/send-payload.js`) and the router server applies the proxy's response headers before the render runs.
