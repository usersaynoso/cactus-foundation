...
1. Assembles the job list - core's own five (`CORE_CRON_JOBS` in `lib/cron/jobs.ts`) plus every installed module's manifest `cronJobs`, read from `Module.manifest`, which `scripts/sync-module-manifests.mjs` rewrites from the deployed `cactus.module.json` on every build. Modules in `failed` or `inactive` status are excluded, as they are from `modules.json`. A manifest entry pointing outside `/api/m/<module-name>/`, or carrying a schedule core cannot evaluate, is logged and skipped - it is never scheduled and never silently half-scheduled.
