...
- **Redirect to the custom domain** - once the combined bootstrap redeploy (below) picks up `SITE_URL`, the wizard's `configure` step needs to send the browser from the Vercel-assigned URL to the real domain (WebAuthn's rpId is derived from `SITE_URL`, so passkey registration fails across a mismatched origin). It reads the target via a fresh `GET /api/setup/env-check` call (`siteUrl` field, read server-side from `process.env.SITE_URL` on every request) rather than the client's `NEXT_PUBLIC_SITE_URL` - that's inlined into the JS bundle at build time, so a wizard tab left open across the mid-session redeploy would otherwise keep comparing against the pre-redeploy value and never redirect.
