...
`scripts/reconcile-core-schema.mjs` closes that gap. It runs on every deploy, immediately after `prisma migrate deploy`, and applies each `.sql` file in `prisma/core-reconcile/` (in lexicographic order) via `prisma db execute`. Every statement in those files is **additive and idempotent** - `CREATE TABLE IF NOT EXISTS`, `ADD COLUMN IF NOT EXISTS`, `CREATE INDEX IF NOT EXISTS`, and foreign keys guarded by a `pg_constraint` name check. On a fresh or up-to-date install every statement is a no-op; on a drifted install it adds exactly the missing objects and nothing else. The files are never used to drop, rename, or retype anything - reconcile only ever moves an old database *forward* to the additive parts of the current core schema.
...
`lib/setup/starterLayouts.ts` does two things, and they are deliberately kept apart:

- **`seedDefaultLayouts(db)` - install only.** Called from `/api/setup/complete` and `reset-database`. Each template flagged `publishByDefault` (core's default header, footer, full-width page, 404 hero, coming-soon and maintenance; plus module templates whose pages have no hardcoded fallback, i.e. Shop's) is seeded as one ordinary published Layout with id `<template-id>-live` and the template's `defaultConditions`. Create-only upsert, so re-running never overwrites the owner's work. The `-live` id suffix is load-bearing: that is where existing installs' live layouts already sit, and renaming it would seed them a second header.
...
The `<id>-live` / `<id>-edited` copies stay a **runtime** job, because deciding them is a content comparison SQL cannot do. The old scheme's migration branch keyed off `status === 'published'` - which every starter row also was - so it minted a `<id>-live` copy of *every* starter on *every* core update, not just the handful a fresh install goes live with, and older versions minted `<id>-edited` copies too. A site that has taken a few updates is carrying dozens of published-but-condition-less layouts that render nowhere. `planStarterCleanup(rows, templates)` (pure, unit-tested in `starterLayouts.test.ts`) deletes a row only when all three hold: its id is `<known-template-id>-live` or `-edited`; its `displayConditions.include` is empty, so it renders nowhere; and its `builderData` still stable-stringifies equal to the canonical template, so nobody has edited it. Anything the owner touched, published, or built fails one of the three and survives. None of those three keys touch the dropped column, so the prune keeps working after the drop.
