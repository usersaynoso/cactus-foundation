...
The proxy therefore also sets **`CDN-Cache-Control`** (RFC 9213), which Next.js has no opinion about and so never rewrites, and which Cloudflare reads *in preference to* `Cache-Control` (precedence: `Cloudflare-CDN-Cache-Control` > `CDN-Cache-Control` > `Cache-Control`). That gives the split the feature wanted anyway: shared caches keep a copy, while the visitor's own browser is still told `no-store` by Next and keeps none. It is kept deliberately plain - `public` and `s-maxage` only - because Cloudflare answers `BYPASS` rather than caching when it meets a directive it will not honour there, and a well-meant extra would silently switch the whole thing off.
...
- `lib/modules/router.public.ts` - everything listed above. Imported only from `app/(public)/**`, `app/sitemap.ts` and `app/robots.txt/route.ts`.
