...
If any installed module has an update available, the confirmation dialog also shows an **"Also update"** checkbox (ticked by default) listing the affected modules and their target versions. Every Vercel build re-clones each module's code regardless of the version pinned in the registry, so a core-only update already refreshes module code on disk - leaving this box ticked lets the installed-version record catch up in the same deploy instead of drifting out of sync with what's actually live (which otherwise left the Modules page showing "update available" for modules that had, in fact, just been updated). Unticking it updates core only, same as before; the modules' own "update available" state is left untouched for you to apply later from the Modules page.
