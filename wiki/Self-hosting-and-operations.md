...
**The download is taken from a single consistent snapshot.** The whole read runs inside one repeatable-read, read-only transaction, so every table, every row and every counter in the file describes the database at the same instant. That matters because a busy site does not hold still while a backup is being written: without it, an order could be read after the counter that numbers orders had already been recorded, or a child record could be written referencing a parent whose table had already been read. Neither shows up when the file is downloaded. Both show up at restore, which is the worst possible moment to find out. The transaction is read-only, so taking a backup cannot change anything, and it is bounded to just under the request's own time limit - on a very large database the download can therefore time out rather than quietly returning a partial file.
...
- **The same build failure repeating on every update** - a module left in `deploying` by a failed build keeps its `pendingVersion`, and every `modules.json` write pins `pendingVersion ?? version`. Before v0.5.1428 that meant an update launched straight after a failure re-pinned the exact tag that broke the last build, indefinitely. Installs, module updates and core updates now settle the previous deployment first: a build that already errored rolls its modules back to their confirmed versions before any pin is written. On an older core, loading the Modules page does the same reconcile - it checks every `deploying` row against its real deployment - so open that page once before updating again.
