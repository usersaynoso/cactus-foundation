...
**Product page address.** If you would rather a product lived at the very top of your site - `/your-product` instead of `/shop/products/your-product` - flip **Product page address** in **Settings → Shop → General → SEO**. Only the product page itself moves; categories, collections, the cart and checkout all stay under `/shop`. Each product then has exactly one address, and the old one forwards to it rather than leaving bookmarked shoppers staring at a not-found page. Everything on your own site follows the setting on its own - the sitemap, menus, product grids, search results, reviews, the accessories panel and the Google Shopping feed - but outside links can still take a while to be recrawled, so it is worth switching deliberately rather than on a whim. If you use the site search, run a reindex afterwards so its results carry the new addresses. A page of your own with the same address always wins the spot - the shop only answers where nothing else does (technically: core resolves an info page first, then a module index, then a module's `publicRootSlug` claim; shop claims a bare slug only when the style is set to root and a product row or forwarding rule carries that slug). The sitemap, menus, product grids, and the Google Shopping feed all follow the setting on their own.

> **Where things live now.** The shop used to spread nine links across your sidebar, and each add-on you installed took another. It now takes two: **Catalogue** and **Trading**.
>
...
> - **Trading** - Orders, Cancellations & returns, Customers, Discounts, Reports, Tax & shipping, Suppliers if you keep them, and Quotes if you have that module.
>
...
`shop.access` really is look but don't touch. Raising or re-sending an invoice or a credit note needs `shop.orders`, and answering, re-opening or deleting a customer's product question needs `shop.products`. The order-size deduction report sits with the other reports, under `shop.reports`.
...
- **PayPal** - same screen: Client ID, Client secret, Webhook ID, plus a Mode switch (sandbox for testing, live once you're ready to take real payments). Same rule - missing keys mean it's silently hidden at checkout.
  ...
  **How a PayPal payment finishes.** The shopper presses **Place order**, approves the payment on PayPal's site, and PayPal sends them straight back to your order confirmation page, which takes the money there and then and shows the order as paid. Cancel on PayPal's site and they land back at checkout with the basket untouched. If PayPal turns the payment down on the way back (a card it won't accept, say), the page says nothing was charged and points them back to checkout to try again or pay another way; the order stays unpaid rather than failed, and is tidied away like any other unpaid order. Try a sandbox order end to end before switching the Mode to live.
- **Bank transfer** - no keys needed. Write your account details on the same tab and orders sit as "awaiting confirmation" until you mark them paid by hand once the money lands. The shopper gets an email as soon as they order carrying those same account details, the amount, and a note that delivery times run from the day the money arrives rather than the day they ordered.
...
**Instant Bank Pay (open banking)** is a fifth option, added by installing the separate [GoCardless Instant Bank Pay for Shop](https://github.com/cactus-foundation-modules/gocardless-instant-bank-pay-for-shop) module. Shoppers authorise a one-off payment straight from their banking app - no card, no stored details. Once installed it joins the list on the **Shop → Payments** tab like any other method, with a button of its own along the top for its settings (add your GoCardless access token and webhook secret, choose sandbox or live). Switch it on, and drag it where you want it, on the list itself. The email address and name typed at checkout are carried over so nobody types them twice, and can still be changed if the payment is coming from an account in another name. Refunds and confirmation work the same as any other method. 

The webhook is the part that actually confirms the money, and it is worth getting right. Note that sites running core older than **v0.5.1436** turned these calls away at the door regardless of how it was set up, so a bank payment could take the money and never confirm the order; if you are updating from further back, look for orders stuck at *awaiting confirmation* and check them against your GoCardless records before chasing anyone. The panel shows the address to paste into your GoCardless dashboard as a webhook endpoint, and the secret you set on that endpoint there has to be the same string as the webhook secret saved here. GoCardless does not check that when you add the endpoint - a mismatched pair looks perfectly healthy from their side and is only turned away when a payment tries to confirm - so the panel now reports how the last delivery went, right underneath the address. Green means GoCardless reached you. Red means it was turned away, and the two secrets differ: until you make them match, payments will sit at *awaiting confirmation* even though the money has left the shopper's account.

There is a second switch on that same panel, **Let shoppers choose their bank on your checkout**, and it is off to begin with. Turn it on and the bank list appears on your own checkout under the payment option, search box and all; the shopper picks their bank there and placing the order takes them straight to it, skipping the two GoCardless screens that would otherwise ask which bank they use and who they are. The approval itself still happens at the bank, because that is the bank's job and nowhere else will do. **It also needs GoCardless to switch it on at their end** - an account upgrade they grant on request, with no setting for it in your GoCardless dashboard, so the only way to get it is to message their support and ask for *custom payment pages*. The panel checks your account and tells you which way round you are. Until they enable it, the switch changes nothing a shopper can see: the checkout carries on sending them to GoCardless to choose, exactly as before.

**Where the card is typed** is a dropdown on that same page, and the one worth a moment's thought.

- **On your own checkout** (the recommended setting) draws Square's own card fields beside your **Place order** button, on the page the shopper is already looking at. The fields are Square's, served from Square and sealed off from the rest of the page, so the card number is no more yours than it was before - the shopper simply never leaves. Pressing **Place order** takes the payment then and there. This needs the Application ID as well as the access token, and the method is quietly not offered until that box is filled in, on the grounds that a card form that cannot be drawn is worse than one that was never promised.
- **On Square's own page** sends the shopper to Square, takes the payment there, and sends them back. This is what the module did before there was a choice, and it is what an existing shop keeps on updating - nothing changes underneath anyone. It is still the right answer if you would rather not think about Application IDs at all.

Either way the order is only created once the card has actually been authorised, so a shopper who thinks better of it leaves nothing in your orders list. The difference is how many of them get that far: a redirect to somebody else's site, in somebody else's colours, is a well-known place for a checkout to lose people.

**Where everything sits at the checkout.** Under the order summary, in this order: anything still holding the button shut, then the wallet buttons, then the card fields, then **Place order**. The card is typed next to the button that spends it, rather than several steps up the page where it was filled in before the total had been seen - and where anyone scrolling straight to the button walked past it. Nothing to set up; it is simply where they are now.

**Apple Pay and Google Pay.** Taking the card on your own checkout used to cost you these, because they came free with Square's own page and nowhere else. Now there is a tickbox for them - **Offer Apple Pay and Google Pay**, on the Square settings page, under the on-page setting - and ticking it puts the wallet buttons directly above your **Place order** button, on the same Square account and the same credentials as the card fields. Nothing else to set up for Google Pay.

Each shopper sees only the one their device actually offers: Apple Pay on an iPhone, iPad or Mac in Safari, Google Pay very nearly everywhere else, both side by side on the rare device that does the two of them, and neither on a device that does neither - in which case they simply carry on typing their card as before. The buttons come to life at the same moment **Place order** does, so nobody skips your terms tickbox or half an address by paying with a thumbprint. Tapping one opens the wallet's own sheet, the shopper approves it there, and the order is placed the instant they do.

Apple wants one extra thing first: proof that this website is yours. In your Square dashboard, register your domain under Apple Pay, download the small file Square hands you, open it in any text editor and paste the contents into the **Apple Pay domain file** box that appears under the tickbox. Until you do, the Apple Pay button stays hidden - Apple would refuse it anyway, and a button that always fails is worse than no button. Google Pay is working the moment the tickbox is ticked.

None of this appears when the card is typed on Square's own page: that page carries Square's own wallet buttons already, and two sets of them on the way to one payment would be silly.

Cards issued in the UK and the EEA generally need the extra bank check (Strong Customer Authentication) before they will go through. On Square's page Square handles that. On your own checkout the module asks for it as part of placing the order, so a shopper's bank may still put its own "confirm this payment" step in front of them - which is the bank's screen, not yours, and exactly as it should be.


**Card payment (Square)** is another card option, added by installing the separate [Square Payments for Shop](https://github.com/cactus-foundation-modules/square-payment-for-shop) module. Card details never touch your site, and you choose where the shopper types them: in Square's card fields drawn on your own checkout, or on Square's hosted page a redirect away. It joins the list on the **Shop → Payments** tab, with a button of its own along the top for its settings. Sandbox and production get their own boxes, so both sets of credentials can be held at once and choosing which one the shop uses is a single dropdown. That dropdown saves itself the moment you change it and takes effect at once - there is nothing to deploy and no Save to press for it. The credentials in the boxes below are a different matter: those are stored with your hosting, so a token you have only just pasted in needs the site to redeploy before it will work. Paste in the access token from your Square developer dashboard, and the Application ID shown next to it if you want the card typed on your own checkout, then press **Look up locations** to fill the location ID in rather than hunting for it. **Credentials only reach your running site when it redeploys**, so a box can quite correctly show *(set)* while the panel above still says Square is not connected. The panel says as much in those words, and names the ones that are saved-but-not-yet-live separately from the ones you have not filled in at all. Deploy from the notification on your dashboard and come back. Add the webhook URL shown on the page to your Square developer dashboard, paste back the signature key it gives you, then switch the method on. The signature key is optional: without it payments are still confirmed when the shopper comes back from Square, but a shopper who pays and then closes the tab has to be confirmed by hand. Refunds and confirmation work the same as any other method.

**Pay by bank (Atoa)** is a second open-banking option, added by installing the separate [Pay With Atoa](Pay-With-Atoa) module. Like Instant Bank Pay it takes a one-off payment the shopper approves in their own banking app, with no card and nothing stored; it is a different provider, so it is a matter of which account you would rather have. You choose where the shopper pays: handed to Atoa's own page and brought back, which is what happens out of the box, or Atoa's payment window opening over your own checkout so they never leave your site - that second one needs your web address whitelisting with Atoa first, and the module's own page explains where. It joins the list on the **Shop → Payments** tab with a button of its own along the top for its settings, where sandbox and production get their own boxes and a single dropdown says which the shop is using - that dropdown takes effect at once, while the credentials in the boxes below wait for the site to redeploy. Registering where Atoa sends its confirmations is a button on that page rather than an address to paste anywhere. One thing is different from the others: **refunds are recorded but not sent**. Pressing Refund takes the items off the order, credits your books and sends the credit note exactly as it always did, but the money itself goes from your bank to the customer's, the same as refunding a bank transfer. Automatic refunds are a later release, and the refund box says so. See [Pay With Atoa](Pay-With-Atoa) for the setup.
...
**No order until somebody has actually paid.** Instant Bank Pay, Pay by bank (Atoa) and Card payment (Square) all finish somewhere else - a banking app, or Square's own page - and plenty of shoppers get that far and think better of it. Those methods now hold everything back until the money is committed, so an order only appears in **Shop → Orders** once there is a payment behind it. A shopper who picks the method, presses **Place order**, and then closes the bank's page leaves nothing behind at all: no order, no number, nothing sitting in your list looking exactly like a sale. Before this, every one of them left a real order marked unpaid, and the only way to tell a genuine one from a change of heart was to wait and see. The one thing you will notice is the odd gap in your order numbers, which is what a walked-away checkout now costs instead of a phantom order. Everything else works as it always did: the confirmation email, the stock, the receipt and the refunds all start from the moment the money lands. The other methods are unchanged - a card typed on your own checkout, or a bank transfer somebody sends you later, still makes its order there and then, because in both cases the order is a real thing the moment it is placed.

**Badges at checkout.** Card (Stripe), PayPal, Card payment (Square), Instant Bank Pay, Pay by bank (Atoa) and bank transfer each show a logo beside the name, so a shopper spots the one they know before reading a word. Nothing to upload: the badge comes with the method, and it is drawn to suit whichever face your site is showing, light or dark. Bank transfer is nobody's brand, so it gets a plain bank building rather than a logo as such. Cash gets nothing - a shopper paying in cash knows what cash is.
...
**Only offering a method on orders of a certain size.** Underneath that, **When each method is offered** gives every method a *Smallest order* and a *Largest order* box. Leave both empty - which is how they all start - and the method is offered on everything, exactly as before. Fill one in and the method only appears on orders of that size. The figure is the total the customer actually pays, VAT and delivery included and any discount taken off, which is the same number printed at the bottom of their order summary. Both ends count, so a largest order of 571 and a smallest order of 571.01 on two different methods hands every order to one of them and neither to both. The usual reason for bothering: one way of taking money is a percentage and another is a flat fee, so there is a figure above which one of them is simply cheaper to be paid by. Some banks and providers also have a ceiling of their own, and a method that was going to be refused at the last step is better not offered at all.

If the basket changes so that the method already chosen no longer qualifies, the checkout unpicks it and says why, rather than letting the shopper find out when they press the button. The rule is enforced again when the order is actually placed, so a page left open in another tab cannot slip past it, and it applies just as much to a customer settling an unpaid order from their own order page later. Set a limit on every method you take and a shopper outside all of them is told there is no way to pay online and to get in touch - so leave at least one method unlimited unless that is genuinely what you want.
...
So an unpaid order offers a second door. On the customer's own order page,
under the bank details, is a short list of the automated methods you already
take - Instant Bank Pay, Pay by bank (Atoa), Card payment (Square) - and the
amount still owed on a button. They pick one, pay it there and then, and the
order carries on exactly as if it had been paid at the checkout: stock,
receipt, invoice and refunds all behave the same. Nothing is taken away in the
process. Your bank details stay on the page for as long as the money is owed,
because plenty of people genuinely want to pay that way, and **Confirm
payment** on your own order screen still does what it always did if the
transfer turns up after all.
...
Two smaller things worth knowing. Only methods that have said they can settle
an order that already exists are offered, which today means Instant Bank Pay,
Pay by bank (Atoa) and Card payment (Square); Stripe and PayPal are not on
that list yet. And a customer who starts a payment and thinks better of it
loses nothing: they come back to their order page with the bank details where
they left them, free to try the other method, or the transfer, or neither.
...
- **Details** - the name, the descriptions, and your own SKU and barcode. Beside them sits a **Supplier SKU** - what the people you buy this from call it, so an order going back to them speaks their language rather than yours. Leave it blank where they use your code. If you've switched on suppliers (see below), the box for picking one lives here too. The web address is set when you first save and left alone after that; rebuilding it to match a new name is a button on the **Search** tab. The full description can stay plain text, or you can hit **Design with the page builder** to lay it out with headings, images and columns instead - and once you're designing, **Open in a new tab** pops that builder out into a page of its own with nothing else on screen, handy on a small window. Saving there sends the design straight back to the product; it never fights with anything you're doing on the main page.
- **Images** - add one or more. The first one is the main photo, so drag them into the order you want (or use the arrows) and the first is what shows on listing cards and in the cart. Each one can have a short description for screen readers and search engines. When you save, your product photos are tidied away into a folder of the product's own, inside a folder named after its lead category (so "Blue Mug" ends up in Shop → Kitchenware → Blue Mug), keeping whatever filenames you uploaded them under. If that lead category sits inside another, the folders nest the same way, so a mug filed under Kitchenware inside Homeware lands in Shop → Homeware → Kitchenware → Blue Mug - the media library mirrors your category tree rather than flattening it. Re-save a product after moving it between categories and its whole folder (photos, 3D models and downloads together) shifts to match. If the product has variations, their photos are filed in a **variations** folder just inside it, so everything for one product sits in one place without a hundred variation shots burying your own three. Housekeeping you never have to think about. A **Media library ↗** button at the top of the tab opens that same folder in the media library in a new tab, for the times you want the full set of library tools rather than the picker (a product with nothing filed yet lands on the nearest folder that does exist). The picker plays along too: choosing an existing image opens in that product's own folder rather than the whole library at once, with a breadcrumb to wander back up and folders to click into if the picture you want lives elsewhere - and the search box looks across every folder regardless. A folder with more pictures than fit on one screenful gets a **Load more** button at the bottom, so nothing is stranded out of reach, and a sort menu lets you order them newest or oldest first, or by name A-Z or Z-A. **Upload new** takes as many files as you like in one go - pick a whole batch and they go up three at a time, the button counting its way through them, and every one that lands is ticked ready to add. Big photographs straight off a camera are fine: they take the same route as the media library's own uploads, so the old "over 4 MB" refusal is gone. If one file in a batch won't go, it says which and why, and the others carry on regardless. On a product with variations, the combinations promoted with **Image up front** appear in this same grid, marked **Variation** (one tile per combination, even where you've picked several of its photos - the caption says how many, and they travel together), and are dragged about with your own pictures - drag one to the front and the product page and its tile in a grid both open on it. Each has its own description box, and its **×** takes it off the gallery without touching the photograph itself - see [Shop variations](Shop-variations#arranging-them-among-your-own-pictures).
...
- **Stock & delivery** - turn on stock tracking to set a stock count and a low-stock warning threshold, and choose what happens when it hits zero: block further sales, or let people order anyway (backorder). **Smallest order** lives here too, for anything you will not sell one of - a box of fifty castor cups, or a chair you will not send fewer than four of. Put the number in and the quantity picker on the product page opens on it, and a basket holding fewer is held back at the checkout with a line saying how many more are needed. It is a **floor, not a batch size**: set it to four and a customer can still order five, or seven - they simply cannot order three. And it is a rule about the **product**, not about one row of the basket: on something with options, four of one colour and four in a mix of four colours both count as four, so a customer is never told to take four of the same thing when they wanted one of each. Leave it empty and things sell one at a time, which is what all but a handful of products want. Pre-order lives here as well: flag a product as a pre-order with an expected dispatch date and an optional note, and customers can buy it straight away while stock only comes off the shelf once you actually mark the order as shipped. So does the weight and size of the thing, which is what postage priced by weight is worked out from - though the weight box only appears while **Charge postage by weight** is switched on under Tax & shipping.
- **Download** - for digital products only, the file customers download after paying, with an optional download limit and expiry. The limit only counts downloads that actually finished, so a transfer that gives up halfway through doesn't quietly cost your customer one of their goes - and opening the link in several tabs at once can't sneak past it. Refund the item (or the whole order) and its link stops working, with a page saying why. Files can be up to 20 MB - the most a customer on an ordinary connection can download inside the minute the download link gets. Anything over 4 MB goes straight to your media storage, so it needs S3-compatible storage (R2, B2 or S3) set up under Settings → Media. The file arrives named as you uploaded it, tidied up where it has to be: the odd character that would confuse a browser is swapped for a space, and accents and other alphabets survive intact.
- **FAQs** - the questions people keep asking about this particular product, and the answers. They show as a section of their own on the product page, below the measurements and above any downloads, with a link in the page's section strip. There's a switch at the foot of the tab for the odd product where the shop's usual answers are wrong: turn it off and the product shows only what you've written here, nothing inherited. See [Frequently asked questions](#frequently-asked-questions) below - most of the writing is better done once for a whole range than a thousand times over.
- **Organisation** - categories, tags and collections, however suits. A product can sit in as many categories as you like, but one of them is its **lead category**, chosen from a dropdown of the ones you've ticked. It's the category the product's photos get filed under. At the foot of the tab sits **Keep this product off the featured shelves**, for the ones you would rather were not paraded on the front page. See below.
...
- **Search** - a custom page title and description for search engines, with a preview of roughly what Google will show. Leave them empty and the product's own name and short description are used. The product's **web address** sits here too, under the preview. Renaming a product deliberately leaves its address where it was - it is a live link, and quietly moving it every time somebody tidies a name would break every share of it - so **Rebuild web address** is there for when you do want it caught up. Press it and the preview shows where the page is about to move to; save and it moves, taking every variation of the product with it, since a variation's address is built from the product's own. The old address forwards to the new one, but it is still worth a thought if you have been handing it out.
...
Both the products list and a product's own edit page carry a **Preview** button - the list's sits beside each row's actions, the edit page's just under Save - opening the live shop page in a new tab. Active products open for anyone; **draft** ones open too, but only while you are signed in to the admin - everyone else gets a page-not-found, and a banner on the page tells you it is still a draft. Archived products grey the button out, since there is nothing to preview.
...
The same shortcut is now on the cards as well, so a category page, a search results page, a collection or any grid of products is a list of edit links to whoever's allowed one. Click the picture or anywhere else on the card and you go to the product page as always; click the name itself, pencil and all, and you land in its editor in a new tab. Handy for working down a category fixing prices, rather than matching rows in the products list to what you were looking at. Again, shoppers get a plain name and nothing else.
...
Every shop answers the same handful of questions until it is blue in the face. How long is the lead time, does it come assembled, can I have it in a different fabric. Those answers can now live on the product page itself, in a section shoppers can open one at a time, with a link in the page's section strip between the measurements and the downloads. Search engines are handed them too, in the form they use for the question-and-answer results you sometimes see under a listing.

You write them in three places, and a product page shows all three at once.
...
**Writing the answer.** Plain writing is all most answers need, and line breaks are kept as you type them. If an answer wants more than that - a link to the returns page, a list of the counties you deliver to, a word in bold - you can type HTML into the answer box and it will be shown as such. Anything unsafe is stripped out on the way to the page, so a pasted block from somewhere else cannot bring a nasty surprise with it. An answer that merely mentions a tag in passing is left alone and printed as the words you typed.
...
**Not inheriting at all.** Both the category panel and the product tab have a tick marked *also show the shared questions*. Untick it and that product, or that whole range, shows only what's written there - nothing from above it. Useful for a range where the standard answers would be actively misleading.

**Nothing written, nothing shown.** A product with no questions of its own, no category questions and no shop-wide ones gets no section and no link in the strip - an empty accordion is worse than none. (The one exception is **Let shoppers ask a question**, below: switch that on and every product gets the section, because the product nobody has written a word about is the one a shopper most needs to ask about.) The same goes for a half-written entry: a question with no answer beside it is simply left off the page. And if you would rather not have the feature at all, the tick at the top of **Product FAQs** in settings takes it off every page in one go, category blocks included.
...
You will never guess every question. **Shop → Settings → General → Product FAQs → Let shoppers ask a question** puts an **Ask a question** button under the questions on every product page. The shopper leaves their name (optional), their email address (not optional - there is nowhere else to send the answer) and the question itself. It is off until you switch it on.

Four boxes sit under the tick: what the button says, the line above the form, what the shopper is told once it has gone, and an address to email each new question to. Leave the last one empty and nothing is emailed - the questions simply wait for you.

Either way, a question arriving lights the bell at the top of the admin: **N product questions waiting**, counting the ones nobody has answered yet, with a button straight to the queue. It follows the queue down as you work through it and takes itself away when the last one is dealt with, so the email address is a convenience rather than the only way you hear about one.

**Answering.** They land under **Shop → Catalogue → Questions**, waiting ones first. Write the answer, press **Email the answer and add it to the FAQs**, and two things happen at once: the person who asked gets it by email, and the question and your answer are added to that product's own FAQs, where the next shopper will find them without asking. That is the point of the whole feature - a question asked once becomes an answer everybody gets.

So write the answer as you want it to read on the page, not as a private note. If a question turns out to be a one-off - a delivery date, somebody's own office - answer it, then take it off the page afterwards: **Delete** on the question does it, or the product's own **FAQs** tab, where it now sits like any question you wrote yourself.

The buttons beside each one: **Bin it** for the spam and the things that belong in an order enquiry, **Put back in the queue** to undo that, and **Delete** to remove the question and the email address with it for good - which is the one to use if somebody asks you to forget them, and the one to use when an answer should never have gone on the page. Deleting takes the answer it published off the product with it, reworded or not, so the page and the queue agree. A question that never reached the page takes nothing off, and the dialog says which of the two you are about to do.

A question marked **Not on the page** was emailed but not added to the FAQs: either that question was already answered there, or it was too long to work as a heading, or the product has since gone. The answer reached the person who asked either way.

**If nothing sends.** Answering needs the site's email set up and the *Product question answered* email switched on under **Settings → Emails**. If either is missing, nothing is sent, nothing is published, and the question stays in the queue exactly as it was, with what you typed still in the box - so it is never a case of wondering whether the customer got it.
...
Rename or move a category - or simply open the Categories screen - and Cactus counts what has been stranded and says so, right at the top, with a **Tidy up filing** button beside it. Press it when it suits you: the files are moved across in small batches, the count on screen keeps up with what has actually finished, and every reference follows automatically, from product galleries and variation swatches to 3D models, downloads, description videos and saved quotes. Choose **Later** and the note simply goes away until you next open the screen.

**Renaming a product is different: its files follow on their own.** Save a product under a new name, or in a different master category, and its whole folder goes with it - photographs, variation pictures, 3D models, spec sheets and swatches together - with the files copied across behind the scenes after the save. A product renamed by a spreadsheet import, or one renamed before this worked, is picked up by the same **Tidy up filing** note instead.
...
Pointing at a tile also shows its second picture on its own, without any clicking, and drops back to the cover when the mouse moves away. That second picture is whatever you actually put second on the product's **Images** tab - the product's own next photo, or a finish you promoted and dragged into second place. Variation colours you haven't put up front stay behind the arrows, so pointing at a tile never flicks it to a colour nobody asked about.

All of that pointing-at business now stays firmly on machines that have something to point with. On a phone there is no such thing as pointing: the first tap was being read as a hover, so the tile lifted, swapped to its second picture and then declined to open the product - and the shopper had to tap a second time to get in. The lift, the picture swap and the little arrow nudge are switched off entirely on touch screens, where nobody was getting them anyway, and one tap opens the product as it always should have.
...
**What a refund does to the rest of the order.** The payment badge follows the money - **Part refunded** or **Refunded** rather than a cheerful **Paid** - whether the refund was made here or in your Stripe or PayPal dashboard, and the **Paid** filter shows the orders that still hold money. Units refunded before they were sent go back into stock; anything already dispatched does not, because a refund is not a returned parcel, so book returned goods back in by hand. On a discounted order the refund box works out each item at what the customer actually paid for it, so a full refund is the order total and not a few pounds more than it.

**Refunding the delivery charge.** The refund box offers the delivery charge too, VAT and all, whenever some of it is left to give back - tick it on its own for a delivery-only refund. It goes on the credit note and comes off the invoice like anything else refunded.

**Refunds made in your Stripe or PayPal dashboard.** The shop hears about these and records them. A refund of the whole payment is recorded as a refund of everything, delivery included - the stock goes back, the credit note is raised and your books are told. A part refund can't say which items it was for, so the order gets a note asking you to record it: open **Refund**, pick the items, and tick **Already refunded** - nothing is sent a second time, and the stock and paperwork follow.

A paid order can't be cancelled from the status menu any more: refund it, and the order follows. Nor can one be marked **Refunded** without a refund on record. Both used to be possible, and both left an order saying one thing while the money said another. The one exception is a part refund you made in your Stripe or PayPal dashboard, which the refund box here can't finish off: an order like that can still be cancelled by hand.

A part refund doesn't stop an order finishing. Two chairs of five refunded before dispatch leaves an order for three, and it's marked complete when they arrive. A part refund on an order that is already complete leaves it complete - the payment badge says **Part refunded**.

If a refund was started and nobody can tell whether it went through, a notice in the admin bell lists the orders to check against your payment provider, and you're emailed when that list changes rather than every hour it stands.

### Orders held when they're paid for

When the money lands and something about the order is wrong, it is put **On hold** with a note on its timeline saying what: the last of the stock had already gone to somebody who paid first, a pre-order went past its limit, or a card or PayPal payment came in for a different amount or currency from the order. The payment is kept either way - nothing is lost - but nobody should send the goods without looking. A coupon used past its limits gets a note and no hold, since the discount has been given and the goods can still go. Held orders are never marked complete automatically; lift the hold when you're happy.

A payment that arrives again after it was reversed - a bank payment that bounced and then went through - is recorded as paid, but the order stays where the reversal left it and nothing is taken from stock or sent to the customer a second time. There's a note on the order saying so.
...
If one does appear: **check the payment with your provider before anything else, and don't refund on the assumption the order is lost.** In most cases the order can be recovered with the payment attached, which is a far better outcome for everybody than a refund and an apology. The warning carries the checkout reference to recover it from, and the technical detail underneath for whoever ends up looking into it.

The same warning turns up for one rarer case: a card payment made from a checkout left open in a tab for more than a day. Unpaid orders are cleared away after a day, so by the time that payment arrives there is no order for it. There's nothing to rebuild it from, so either refund it in your provider's account or take the order again by hand and keep the payment.

Once you've dealt with one - refunded it, or taken the order again - press **I have dealt with this** to take it off the list. That's the only way off for a payment settled some other way, and the list is the only record the shop keeps that the money arrived, so it asks first.
...
**Export CSV** hands you the orders you're currently looking at, filters and all, one row each - up to 5,000 at a time. Handy for the accountant, and rather easier than copying numbers off the screen by hand. If the filters match more than that, the file is named for it (`first-5000-of-6123`), so a partial export can't pass itself off as the lot; narrow the dates and export again for the rest.
...
**Items and the money.** Each item's name opens the page the customer bought it from, in a new tab - for a variation, the product page already set to the size and colour they chose, rather than a note telling you to go and edit it somewhere else. Where a delivery service is priced per item (the delivery tiers from Advanced Shipping, say), that delivery used to be folded into the item's price, and the totals then swore delivery was £0.00. The item now shows its own price, the delivery sits beside the delivery line it pays for, and the totals read goods subtotal, then delivery, then tax - the same split the basket and the invoice already made. Nothing about what was charged has moved; it is the same money, finally in the right rows.
...
It is editable like everything else under **Settings → Email → Templates**, including the lead-time line if you would rather it went. There is a short text version too if you have texts switched on, which points at the email rather than trying to fit account numbers into a text message. Card, PayPal, Instant Bank Pay and Pay by bank (Atoa) orders are unaffected - their money arrives in seconds, so the confirmation email covers it.

You get told as well. A **New order awaiting payment** alert goes to your usual new-order address the moment the order is placed, saying in as many words that the money isn't in yet and the order is sitting in Awaiting payment until you say otherwise - so a bank transfer no longer arrives in silence and sits in the list hoping somebody scrolls past it. It's a separate message from the ordinary new-order alert on purpose: one means "get it out of the door", the other means "somebody owes you money", and packing the second one on sight would be jumping the gun. You won't get a second alert when you mark it paid - you'll be looking at the order at the time, having just pressed the button.

**Marking a bank transfer or a cash sale paid.** Money that arrives by bank transfer arrives quietly, so those orders wait in your list until you say the money has landed. Press **Payment received** on the order and you get a tick box: **Email the customer to say the payment has arrived**. Leave it ticked and they get the **Payment received** message - the amount, the reference if there is one, what they bought and where it is going, and a line saying you are getting it ready. Untick it if you have already told them yourself on the phone, and the order goes through in silence. Either way the stock comes off, any downloads are released and the invoice is raised exactly as before.

That message is editable like every other one, under **Settings → Email → Templates**, and if you have texts switched on there is a short version of it too. It has taken the place of the order confirmation that used to go out at that moment, which had been telling people their order was confirmed when what they were actually waiting to hear was that their money had turned up.
...
Once recorded, each item shows how many have gone and how many are still to follow, a **Parcels** list appears with every parcel so far, and the badges at the top of the order read **Partly dispatched** or **All dispatched** - and **All delivered** once everything has gone and the courier says every parcel has landed. That badge is worked out from what's actually left the building rather than being a status you set, so it can't drift out of step with reality and there's no extra status to remember to change. The same badge shows on the orders list, and you can filter the whole list by it.

**When the next van is due.** On the orders list, the Dispatch column also shows **Delivery due Tue 22 Sep** under the badge - the soonest day anything on the order is due at the customer's door. Two things feed it:

- **Items not sent yet** are due on the day the customer was promised at the checkout - the "Arrives by" date their delivery service quoted. So an order you haven't touched yet already shows when it has to be there, which is rather the point of looking.
- **Parcels on their way** are due on the **Delivery date** typed into **Dispatch items** (or edited on the parcel afterwards). The courier's day wins over the checkout promise for whatever is in that parcel, being the later and better-informed of the two; a parcel with no day booked yet falls back to its items' promise.

A parcel that has already been delivered is skipped, so an order split across Tuesday and Friday reads Tuesday until Tuesday's has landed and Friday after that. Nothing shows once an order is completed, cancelled or refunded, nor on an order still waiting for its payment - until the money arrives the promise is a number of working days rather than a date. The checkout promise comes from whichever module quoted it (Advanced Shipping, on most shops); a shop without one shows only the parcels' booked days.
...
Ticking **Email the customer** sends the **Part of an order dispatched** message, which lists what's in this parcel and what's still to come, and reassures them that separately sent parcels often arrive a day or two apart. When the parcel is the one that finishes off an order already partly sent, the same email says so instead. Where the parcel *is* the whole order - nothing sent before it and nothing left owing, which is every order that goes out in one go - the customer gets the ordinary **Order shipped** message instead, because being told "the last part of your order is on its way" when there was only ever one part reads like an earlier parcel has gone missing. Both are editable under **Settings → Email → Templates**.

**The order's status follows along.** When the parcel you record is the one that leaves nothing owing, an order still marked **Processing** moves on to **Dispatched** by itself, so the customer's order page says **On its way** instead of **Being prepared** to somebody who has just been emailed the opposite. It happens quietly - the parcel's own email is the one the customer gets, and an order set to invoice on dispatch is invoiced exactly as if you had changed the dropdown. Orders you have put **On hold**, or already marked **Complete**, are left alone.

Recorded a dispatch that never happened? Press **Undo** beside it in the Parcels list and the items go straight back to being outstanding. Nothing else needs correcting, because the totals are counted from the parcels rather than kept in a separate tally that could disagree with them. If that undo leaves a **Dispatched** order with something still to send, it goes back to **Processing**, without emailing anybody about it.
...
**Your couriers, as a list.** Under **Shop → Tax & shipping → Couriers**, add the couriers you actually use. Each one carries a name, a set of delivery questions, and one tick box: **Show customers this courier's own tracking page**. Leave the list empty and dispatch keeps the plain **Carrier** box it has always had, and nothing changes for you.
...
**The delivery date.** In **Dispatch items** there is now a **Delivery date**, and next to it a **window starts** and **window ends** if the courier has already told you. The customer's order page reads **Arranged for tomorrow between 10am and 1pm** - the day named the way somebody would say it out loud. Today and tomorrow get their own words, the rest of the week is named by its weekday, and anything further out gets the date. The times are spoken too: 10am and 1pm rather than 10:00 and 13:00. The delivery email is the one place that always spells the date out in full, because "tomorrow" in an inbox is wrong by breakfast.
...
**Following the courier automatically.** Some couriers publish a tracking page the shop can read for itself. On those, set **Follow this courier's tracking automatically** and the shop checks each live parcel once an hour - and again whenever the customer opens their order, at most once every five minutes per parcel however often they reload - keeps the answer on your own site, and never sends the customer anywhere near the courier's page.
...
**DPD: one link, and it has to be the right one.** For a courier whose tracking is read from DPD, the parcel takes a single tracking link - the follow-my-parcel link DPD email once they have the parcel, which looks like `https://www.dpd.co.uk/d/6dPoGvP3DMDN`. Nothing else is accepted for DPD: the long `track.dpd.co.uk/parcels/…` page, a bare code or anybody else's link is refused at the desk, with the box turning red as you type. That one link is all it takes - the shop works out the parcel number from it and reads the delivery window, the driver's name and the drop numbers above. It usually turns up the day before delivery, so it normally goes in by editing the parcel rather than at dispatch. Any other courier still takes any web address as its tracking link.

**When the courier could not deliver.** Couriers do not always get in. Each courier you follow automatically has a third list alongside the out-for-delivery and arrived ones: **stages that mean the delivery failed**. The start of the courier's wording is enough - "Failed Attempt" catches "Failed Attempt - Non Fault - Recipient Not Home" and whatever reason they staple on next time. When a parcel reaches one, the customer's order page stops saying "Out for delivery", the van and the map go, the rail reads **Delivery not possible**, and in place of the booked day the customer is told to get in touch with the courier to arrange a new one - by the **live chat link** and **phone number** you give that courier, with their tracking number printed alongside, because it is the first thing the courier will ask for. They are also sent one email saying the same - **Delivery not possible** under Settings → Emails, where you can reword it. One per failed attempt: a second failure on the same parcel is a second email, the hourly re-check of the same failure is not.

**Who books the new day** is a setting on each courier. *The customer gets in touch with the courier* (the default) is what is described above. *The courier gets in touch with the customer* tells them instead that there is nothing to do and the courier will be in touch to arrange a new day, with the chat link and phone number underneath for anybody who would rather get hold of them sooner - on the order page and in the email alike.

**The courier's reason** is kept back unless you tick **Tell the customer the courier's reason for a failed delivery** on that courier. Ticked, whatever the courier adds after the failed stage is shown on the order page and in the email, tidied up: "Failed Attempt - Non Fault - RECIPIENT NOT HOME - UNABLE TO DELIVER" becomes "Recipient not home - unable to deliver". It is off by default because it is written for drivers, and "Recipient not home" can read as an accusation to somebody who was in the garden.

On a courier set to *the customer gets in touch*, you may have already rung the courier and been told *they* will call the customer. On the order screen, a failed parcel shows what the customer is being told and a button to change it: **[Courier] will contact the customer**. Press it and the customer's page says the courier is arranging a new day and will be in touch, and that there is nothing they need to do; press it again to go back. It clears itself the moment the courier's tracking moves on - a new day booked, or another failed attempt - so an old "they will ring you" never hangs over a new problem. On a courier set to *the courier gets in touch*, the order screen just says so and there is no button. The button sends nothing itself. The email usually goes before anybody has had the chance to press it, so it will normally be the "get in touch" version; if the button was pressed first, the email says to wait instead.

**Redelivery charges.** When a courier charges you for another attempt because nobody was in, you can pass that on. On the order screen, **Redelivery charge → Charge for redelivery**: the redelivery fee before VAT, the VAT rate (it starts at the order's own rate), an optional **cancellation charge** before VAT, and, if you like, a note. The cancellation charge is only taken if the customer cancels instead of paying; give it a reason in **Why there is a cancellation charge** ("this is what our supplier charges us to deal with a cancelled order - we pass it on at cost"), which the customer sees beside it. The box starts with whatever you wrote last time. By default the order goes **on hold** and the customer is emailed (**Redelivery fee to pay** under Settings → Emails).

On their order page the customer then sees the fee with two ways out of it:

- **Pay it** there and then, by card - through any payment method you have switched on that can take a payment separately from the order (Square, for now). The order comes off hold, back to where it was, and you are emailed to say so; booking the new day is still yours to do. If no method can take it online, the customer is asked to get in touch, and you press **Record as paid** once the money is in.
- **Cancel instead**. The page says plainly that the first attempt has already happened and the courier has charged for it, so the redelivery fee is still due if they cancel - plus the cancellation charge and your reason for it, where there is one. They are refunded what they paid **less both**. The refund goes straight back the way they paid, the order is cancelled, both of you are emailed, and the credit note is for the smaller amount, so the charges stay on the invoice, VAT and all. Getting the goods back from the courier is up to you.

While the fee is waiting you can **Change** it - the fee, the VAT rate, the cancellation charge, either note. The customer is **not** emailed unless you tick **Email the customer the new figures**; their order page shows the new figures straight away either way. Changing the amount to pay retires the old figure and raises a new one, so a card payment somebody had already started at the old amount cannot quietly settle the new one - if one lands, a note on the order's history says so and leaves it to you. From the order screen you can also cancel or record payment on the customer's behalf (for the customer who rings rather than clicks), send the email again, or **Waive** the fee. One charge can be waiting on an order at a time. A fee that is paid is paid separately from the order, so it is not on the order's invoice - a note on the order's history says so, and it wants recording in your books.

**Your own "Redelivery fee to pay" email.** If you have reworded it, the new merge tags are `{{cancellationTotal}}`, `{{hasCancellationCharge}}`, `{{cancellationNote}}`, `{{hasCancellationNote}}`, `{{keptTotal}}` (everything kept if they cancel) and `{{paidAmount}}`; your wording is left alone, so add them yourself if you charge for cancelling.
...
Optimise it, resize it or move it in the library like any other picture and the order follows it - the parcel on the order page, and a customer's photographs of a damaged delivery, point at the file's new home straight away. (They used not to. Optimising a signature quietly left the order pointing at a file that had just been swapped for a smaller one, which is a poor time to discover your evidence has gone walkabout.)
...
If **Show it to the customer on their own order page** is ticked, the invoice appears as a link on their order in their account, beside the printable receipt. Clicking it saves the PDF straight away - somebody opening their own paperwork wants the file, not another page to save it from. The link is signed, so it works from an email without anybody signing in, and it carries nothing personal in the address itself.

Before an invoice has been raised, that spot says when one is coming - "Your VAT invoice will be available on completion of your order", or whichever moment you issue on. It saves the email asking where it is.
...
- **A minimum order value is measured before its own discount comes off** - otherwise "£10 off orders over £50" could never be used on a £55 basket - but after any discount applied before it. The coupon goes first, then the automatic discounts in priority order.
- **A free-delivery threshold is measured on the goods after discounts,** the same way a delivery rate's own "free over" figure is. A coupon that takes a £110 basket to £80 no longer collects free delivery "over £100", and a free-shipping rule with a threshold waits for it rather than giving free delivery on everything.
...
A file can be up to 20 MB, which is a very long catalogue (anything over 4 MB goes straight to your media storage, which needs R2, B2 or S3 set up under Settings → Media). A big import that runs out of time stops cleanly at a row and says which one: everything before it is saved, and uploading the same file again carries on from there, because rows already in match and change nothing. An import that dies part-way no longer sits in the log saying it's still running for ever - after ten minutes it's shown as failed.
...
A shop that **has** zones but none left empty to catch everything is making that decision, though. A postcode none of them covers used to go through with no delivery charge and no VAT, which is the most expensive way to find out a zone was missing. It now gets the same "we can't deliver there" message as an excluded one, but only when the basket has something physical in it - a download goes anywhere. **Tax & shipping** warns you when no zone catches everything, so you can decide whether that's what you meant.
...
**The Tax report** (**Shop → Reports → Tax**) gives each rate's tax collected, tax refunded and tax owed for any dates you pick - collected on the day each order was paid for, refunds on the day they were made, days starting at midnight in your site's timezone. Delivery VAT is included, split across the rates the way your invoices split it, and the CSV follows the dates you picked.
...
Guests aren't shut out of their own order either - see below.

### Customers without an account

A guest who checked out without signing up used to have nowhere to go. Their order existed, their invoice existed, their parcel was moving, and none of it was anywhere they could look - so every one of those questions arrived by telephone or email instead, including the awkward one where their accounts department has just told them the invoice needs to be in the holding company's name.

They can now open their own order by proving the **postcode it's being delivered to**. No account, no password, nothing to remember. What they get afterwards is the same order page a signed-in customer gets: where it has got to, what's in it, their invoice and credit notes, their parcels and tracking links, anything still to pay, and the same box for putting right the company and address the invoice is made out to.

There are two ways in.

**From an email.** Every email you send about an order now carries a line: *Keep track of your order at ...*. Someone already signed in and looking at their own order goes straight through to it; anyone else is shown the order number and asked for the postcode. That decision is made when the link is clicked rather than when the email was sent, so a guest who creates an account a fortnight later finds the same old link takes them straight in.

**From the tracking page.** `/shop/track-order` asks for an order number and the delivery postcode. You can give it a short address at the root of your site as well - **track-order** out of the box, so `yoursite.co.uk/track-order` - which is the version worth printing on a delivery note or reading out on the telephone. Leave that box empty and only the longer address exists. It also asks whether they'd like an account, since the whole thing is a slower version of having one.

Both are forgiving about what gets typed. Spaces and capitals in a postcode make no difference - `E1 1AA`, `e1 1aa` and `E11AA` are the same postcode - and an order number works with or without your prefix and without its leading zeros, so `DW000172`, `000172` and plain `172` all find the same order. `B29 7QB` still finds nothing, which is rather the point.

Once someone has proved a postcode, that browser remembers it for **30 days**, for that order and no other. Tracking two parcels means proving two postcodes.

**It is not a guessable lock.** Order numbers run in sequence and are printed on every email you send, so the postcode is the only real secret in the pair - and Cactus treats it that way. Eight wrong postcodes on one order and that order stops answering for an hour, however many different computers the guesses come from. Every kind of failure gives the same reply, so nobody can use the failures to work out which order numbers are real, and the link in the email is never a way in on its own: these emails get forwarded to partners, accounts departments and whoever actually presses the button at the bank, and none of them should get the delivery address thrown in.
...
- Texts cover the customer-facing milestones only: order confirmed, order placed with payment still to come, payment received (bank transfer and cash), being processed, dispatched, complete, cancelled, part of an order dispatched, and the three about a cancel or return request. Your own alerts - new order, low stock, import finished - stay on email, on the grounds that you're sitting at a screen and texts cost money.
...
**The order page reads top to bottom in the order people actually want it.** The order number, its state, the date, the number of items and the total sit at the top, and directly under them is a **progress strip** - Ordered, Paid, Dispatched, Complete - with a tick against everything that has happened and the step it's on picked out. Somebody who opens the page to ask "where is it?" has the answer before they've scrolled. Under that comes anything they still have to do about it: money owed, or a cancellation they've asked for and haven't heard back on. Then the receipt itself, with the items and the totals in one box rather than at opposite ends of the page. Everything after that - parcels, downloads, paperwork, how it was paid, the two addresses, refunds, past requests - is a set of smaller boxes that sit **two side by side on a computer and one above the other on a phone**, so a long order stops being a mile of scrolling.

Two details worth knowing. **Things bought together stay together**: a desk and the drawer unit ordered with it sit as one group with the accessory tucked under its parent, the same way they did in the basket and on the thank-you page. And **where somebody bought more than one of something the single price is shown** as well as the line total, rather than leaving them to divide one by the other. The paperwork - the printable receipt, every invoice including any that were cancelled and replaced, the proforma, any credit notes - now has a box of its own with a row per document, instead of being a run of links trailing off the date.
...
The **Addresses & Phone Numbers** tab fills itself in. Every address a signed-in customer orders to is kept there once the order is placed - the billing address too, where they gave a separate one - so the second order asks them for rather less than the first one did. They can add one by hand too, and delete any of them, but nobody has to remember to save anything.

Each saved address has an **Edit** button, which opens the whole address for changing - names, phone number, both address lines, town and postcode - rather than only the phone number. A company that's moved office edits the address it already has instead of adding the new one and deleting the old one. The **Add address** button opens the same form, empty. Both are the checkout's own form, down to the labels and the messages when something's missing, and both will look an address up for you where the checkout does (see below).
...
Guests get the same safety net on the machine they're using. Their basket is kept on the shop as well as in the browser, so a cleared cache, a crashed tab or a browser having a tidy-up doesn't take the shopping with it. It doesn't hop between their devices - there's no account to hang it on and the shop has no business guessing - but it survives on the one they've got.

Nothing personal goes with it. The basket kept for a guest is a list of what they picked and nothing else: no name, no email, no phone number, no delivery address. Anything typed into the checkout stays in their own browser until they actually place an order. The shop sets one small cookie so it knows which basket is theirs, which is the shopping-basket cookie the cookie rules specifically say you don't have to ask about - and it earns that by doing nothing else. A guest basket nobody comes back to is thrown away after a couple of months.

It's worth saying that a basket is a list of what somebody fancied, not a promise about the price: what things cost, what's in stock and what delivery comes to are all worked out fresh when the basket is opened and again at the checkout, so a basket left for a fortnight can't hold you to a fortnight-old price.

There's nothing to switch on, for either kind of shopper. The account-to-account part needs [Members](Members) running; the rest is simply how baskets work now.
...
The rules are the sensible ones and you don't have to police them: **cancelling** is only offered on things that haven't been dispatched (once something is in a van, that one's a return), and **returning** is only offered after something has actually gone out, within a window you set. That window is counted from the day the last parcel left, not the day they ordered - an order that sat on your shelf for three weeks shouldn't eat the customer's return window. One open request per order, and they can withdraw it if they change their mind.

**They can call off part of an order, not just all of it.** Somebody who ordered five chairs and only needs three says so on the page: the form comes up with everything that can still be stopped already ticked, and they change the numbers if they only want some of it gone. Press send without touching anything and it's the whole order, exactly as before.

Anything that can't be stopped is listed underneath with the reason - already on its way, or something you don't take back - rather than quietly left off the form. An order showing three of its five lines and no explanation reads as broken, and the customer's next move is an email asking about the other two.
...
**One thing that follows from "do not take it back".** A line you will not take back cannot be called off through the website either, even before it has been dispatched. Goods like that are cut, upholstered or ordered in specially the moment the order lands, and the van not having been yet does not put them back in the box. It is per line, not per order: on an order holding one bespoke desk and three stock chairs the chairs can still be stopped, the desk is listed underneath with your reason beside it, and only an order where nothing at all can be stopped says so instead of offering the button.
...
**Not asking for a review on an order that went badly.** If your **Order completed** email asks for a Google review (or any other kind), wrap that paragraph - and anything else that only makes sense on a happy order, a P.S. with the link again, say - in `{{#if hasReviewRequest}} ... {{/if}}`. Every order asks by default. On an order where asking for five stars would be pouring salt in it, untick **Ask for a review in the completion email** under **Status** on the order's own page; it saves the moment you untick it, so it still counts if the order then completes itself when the courier reports it delivered. The rest of the completion email, invoice and all, goes out as normal. The same flag works in the completion text message, for anyone who put a review link in that. It is not offered on a replacement, whose last email is its own and asks for nothing.
...
Those photographs are also filed in your media library, under a folder named for the order (Orders → the order number → issues), so if you ever need to find them again outside the queue itself, that is where they are. Only photographs uploaded to that order's folder are attached to its report, so a report can't borrow a picture from somewhere else in your library. A fully refunded order no longer offers the button; one refunded in part still does, since the refund may have been for something else entirely.
...
**On the order itself, too.** Open an order a customer has reported something on and it says so: a **Customer report waiting** badge under the order number, and a **Reported by the customer** card with the whole story - damage, return or cancellation, the reason they picked, which items it's about, what they wrote, and every photograph they sent, each opening full size with a click. While anything is still waiting the card sits at the top, above the items, because a photo of a snapped chair leg is usually why you opened the order in the first place; once it's settled it moves down below the parcels and shows what you replied and any replacement you sent. The report and its outcome also appear in the order's timeline. Deciding still happens in the queue, where the refund and replacement steps live - the **Decide** button takes you there.

The photographs follow the media library. Optimise one, resize it or file it somewhere else and the order shows it at its new address straight away, rather than a broken image where the evidence used to be.

Approving a cancellation that covers the whole order also closes the order, so it can't be picked and packed by mistake while you're sorting out the refund. Approving one that covers part of it leaves the order open - five chairs less two is still an order for three - and takes the called-off units off the dispatch screen so they can't go out by mistake. That happens whether or not you refund on the spot, which is the point: on a shop that settles by hand, "approved on Monday, paid on Friday" must not mean all five turn up on Tuesday.
...
The refund sends back what the customer paid for those items: VAT included on a shop that adds it at the till, less their share of any discount. When the whole order is cancelled before anything has been sent, the delivery charge goes back too, as UK cancellation rules expect; otherwise it stays, and you can refund it from the order if it should go. The box is only ticked to start with when money was actually taken.

**A whole-order cancellation can't be approved once part of it has gone.** If some of the order was sent while the request was waiting, approving it is refused with a note to decline it, refund what hasn't gone and arrange a return for what has. A cancellation that names particular items only ever refunds units still on the shelf.

**Dispatch knows about waiting requests.** While a customer's cancellation or return is still waiting for your decision, **Dispatch items** says so, line by line, so a chair somebody has asked to call off isn't packed while the request sits in the queue. It's a warning rather than a lock: an ask isn't an approval.
...
**Linking back to the original order.** Every one of these messages names the original order number, and can carry a link straight to its page as well: `{{parentOrderUrl}}`, wrapped in `{{#if hasParentOrderUrl}} ... {{/if}}` like any other optional line. It is worth having, because on a replacement `{{orderUrl}}` points at the replacement's own page - one line and a parcel - while the invoice, the delivery address and the photographs they sent you are all on the original. The four replacement emails carry the line already; the text messages are offered the same wording but left without it, since a second link is most of a text message. An email you have already rewritten keeps your own wording, so add the tag from the chip list above the editor when you want it.
...
**Keeping a list of parts.** Tick **This is a spare part** on any product and it drops out of the shop entirely - no grids, no search, no product page, no checkout, nothing for Google to find - while keeping its stock figure, its cost price and its supplier code. Staff can still open its page (with a note saying shoppers can't) and put it on an order entered by hand. You pick it by name when you send a replacement, the stock comes off when you raise the replacement, and what your warranty actually costs you is a figure rather than a feeling. You don't have to: the box also takes anything you type, which is the right answer for a fixings pack you'll never send again.
...
The **Category** tab has two blocks worth knowing about. **Category Browser** shows a category's sub-categories as cards - or, with its "Show as" setting flipped to **Pills**, as a compact row of wrapping link chips, which suits pages where the sub-categories are shortcuts above a product grid rather than the main event. On a phone that row stops wrapping and becomes a single strip you swipe sideways: four sub-category names used to stack into four full-width rows, which is a lot of signposting to scroll past before reaching anything you can actually buy. The row also fades out at whichever edge has more waiting past it, with a small arrow to tap for anyone who would rather not swipe - the same treatment the admin tabs get. If a category has a great many sub-categories, the **Pills: show only this many** setting caps the strip: put in 6 and the six busiest print, counted by how many products each one holds (including everything filed in their own sub-categories), with the rest tucked behind a **6 more** button that opens them where they sit. Leave it at 0 and you get the lot, exactly as before. Nothing is actually removed from the page - every sub-category link is still there for search engines to follow, it's just folded up until a shopper asks. Drop it on the Category design and it works out which category it's on by itself, so one design serves the lot. (It's the same block you can put on Shop Home to show your top-level categories.) **Category Description** prints whatever you've written for the category - the designed version if you've built one, the plain paragraph if you haven't, and nothing at all if you've written neither, so a category you haven't got round to yet doesn't leave a hole. A plain written-up description now folds away behind a **Read more** that sits on the end of the short blurb under the heading, on every screen rather than only on a phone: the page reads heading, one opening line, the link on the end of it, then the products. Click it and the description opens where it always sat; click **Show less** and it tucks back. If you have turned the blurb off, or never wrote one, the link keeps its old place just below. Nobody has to scroll past a wall of prose to reach the stock, and search engines are handed the whole thing either way. A description you've *designed* in the builder is left exactly as you built it, on the grounds that hiding someone's careful layout behind a link is not our call to make - if a designed description needs to be shorter, the builder's own device settings will do it properly. There's a starter called **Sub-categories First** that puts the two in the sensible order: heading, sub-category cards, description, then the products.
...
The buy row also copes better with a genuinely narrow column. When there isn't room to stand the quantity picker and the button side by side, the button drops onto its own line - and the quantity picker, rather than sitting stranded at its natural width with half the row empty beside it, now stretches to the full width with the count spread out in the middle. Bigger target, tidier row, and no setting to find: it happens on whatever width actually runs out of room, be that a phone or a narrow column you have designed yourself.
...
The row also ends with a filled button of its own, so the shopper never has to hunt for the buy button after tapping their way down the page. On a straightforward product it says **Add to basket** and pops the item straight in the basket from wherever they're reading. On a product sold in different versions - a colour, a size, that sort of thing - it says **Configure** and jumps them straight to the option pickers themselves, since there's nothing to add until they've chosen. And on a phone, where the product photo or 3D view pins itself to the top of the screen while the shopper picks their options, the media strip takes the top spot over the tab row so the thing being configured stays visible.

The **Dimensions** section is the one part you can now show the door. Some products carry a full set of measurements worth spelling out; others have a stray height and nothing else, or repeat what the specification table already says, and the section earns its place on neither. Each of the three pieces carries the same switch - **Show Dimensions tab** on the Tabs piece, **Show Dimensions section** on the Sections piece, **Show Dimensions link** on the Section links piece - so set them all to **No** to take it off the page entirely. Set only the Tabs one and the measurements stay on the page with nothing pointing at them, which is rarely what anyone wants. Everything ships switched on, so nothing changes until you say so, and a product with no measurements recorded never showed the section in the first place.

A **FAQs** section joins the run whenever the product has questions to answer - its own, its category's, or the shop's - sitting after the measurements and before any downloads, with a link of its own in the row. There's nothing to switch on here: a product with no questions gets no section and no link, and one with questions gets both. See [Frequently asked questions](#frequently-asked-questions) for where the answers are written.
...
On a phone, product grids now show **two products across** rather than one. A single card per row turned a category into a very long scroll indeed, with most of the shop hiding below the fold. Because each card is half the width it was, the wording on it - the name, the price, the option swatches, the link - steps down a little to match, and over-photo badges do the same, so the card still sits neatly instead of turning into a label collection with a product somewhere underneath. The photos keep their shape and take the room the words gave up. Nothing changes on a tablet or a desktop.
...
- The **product grid** can order its shelf: newest first (as always), **New tag first, then newest** (everything carrying the `new` tag at the front, each group still ordered by when it was added - the usual answer for a "Just in" row), best sellers first, price in either direction, or by name. It can carry a **heading** and a smaller line beside it, and you can reword the message shown when a shelf turns out to be empty.
...
- Every **checkout step heading** is editable - Contact details, Delivery address, Delivery method, Payment method, Order review - along with the order summary's heading and its **Edit basket** link, the wording on the **Place order** button (the total stays on it no matter what you call it), and the reassurance line beneath it - which is now blank to begin with. It used to ship with a padlock and a sentence about card details being encrypted, which was neither here nor there on a checkout taking a bank transfer, and read as filler on the ones where it was true. Write your own line there if you want one, or leave it empty and the space closes up.
...
- The **product grid** can carry a **View all** link on the far end of its heading, for a shelf that is a sample of something bigger - a "Best sellers" row of four on the home page, say. Switch it on, reword it if you like, and leave the address blank: the block works out where it goes from whatever the grid is already showing, so a grid cut to a tag, category, collection or supplier links through to that page on its own. A grid showing a slice of the whole shop has no such page until you build one, so type the address in yourself - a filter page (see the Filters add-on) is the usual answer, since it gives shoppers the whole list with the filter panel beside it. The link lives in the heading strip, so a grid with no heading shows none.
- Both the **product grid** and the **featured collection** carry a **Products kept off the featured shelves** setting, set to leave them out. Flip it to **Show them here anyway** where you are using the block as a plain catalogue listing rather than a showcase row. See **Keeping a product off the featured shelves** below.

### Keeping a product off the featured shelves

Some products earn their place in the catalogue without deserving the front page. A clearance line you would rather not lead with, a bulky thing you only sell to trade, an oddity that keeps winning "best seller" on the strength of one enormous order. **Organisation → Keep this product off the featured shelves** takes it out of the showcase rows without taking it out of the shop.

Tick it and the product stops appearing on the shelves you have dropped onto your pages: Best sellers, Just in, Staff picks, On offer, whatever you have called them. Everything else carries on exactly as it was. It still sits in its categories and collections, still carries its tags, still turns up in search and in your sitemap, still has its own page and can still be bought. It is a decision about the shop window, not about the stockroom.

The shelves it applies to are the **product grid** and the **featured collection** blocks, each of which starts out leaving flagged products out. Where you have used one of those blocks as an ordinary listing rather than a showcase - a "everything in stock" page built out of a product grid, say - open the block and set **Products kept off the featured shelves** to **Show them here anyway**, and that one block goes back to showing the lot.

Nothing changes until you tick something: on the day this arrives no product is flagged, so every shelf shows exactly what it showed yesterday.

The tick also travels on the products spreadsheet, as a `featured_hidden` column reading `true` or `false`, so a shopful of them can be set in one go rather than one product at a time. A spreadsheet from before the column existed is still a perfectly good spreadsheet: the column is simply left alone.
...
The checkout used to keep the shopper's actual order rather quiet: a totals table at the bottom and not a word about what was being bought. It now opens with **Your order** - every item with its photo, its quantity, its price and, crucially, its own choices spelled out underneath. Any whole-basket promise ("Everything gets to you by Fri 4 Sep") sits under the list, and an **Edit basket** link takes the shopper back to the cart if something needs changing - checkout itself stays a straight line to the finish.
...
That summary is its own block, **Shop: Checkout - Order summary**, and the ready-made checkout designs all include it: the single-column ones put it first, so the shopper starts by seeing what they're buying, and the two-column one puts the whole summary down the left with the checkout steps alongside on the right - and the summary can be set to stay put while the page scrolls, so the order never disappears off the top while someone types their address. A summary too long to fit on screen sorts itself out: it runs down the page at its full height, so ordinary scrolling shows every item in turn, and once its last line is in view it pins by its foot and keeps the end of the order - the bit nearest the Place order button - in sight while the fields are filled in. A summary short enough to fit simply pins near the top as it always did. Earlier versions tried something cleverer here, taking over the page's scrolling to feed the order past a pinned summary, and clever is exactly what it turned out to be: on the wrong day it froze the page mid-scroll, drew the checkout over the footer, and let the page scroll on past the footer into nothing. All of that machinery is gone. The **Scroll a long order inside the block** setting remains for the owner who would rather have a compact box: **Always** caps the list and scrolls it inside the block (put the mouse over it and scroll; leave the height empty and it takes as much of the window as it can under whatever sticky offset you set, or name a length if your own header maths says otherwise), while **Automatic** and **Never** both let the order run down the page and rely on the foot-pinning above. On a phone none of it applies - the page does the scrolling, as it should. On a phone the summary starts folded away behind a Show link (the item count and total stay visible), because the fields are what a phone-sized screen is really there for. And a basket whose items have since been removed from the shop now says so plainly, rather than presenting a heading over thin air. The checkout page itself now gives a published design proper room to spread out, rather than squeezing everything into one narrow middle column. An existing published checkout design won't change by itself - open **Appearance → Layouts → Shop → Checkout** and drop the block in wherever suits, or pick a fresh starter.

The delivery address can look itself up, too. With the [Address Lookup](Address-Lookup) module installed (and a key entered for whichever service you have chosen, Ideal Postcodes or Google), the shopper types the first line of their address into the ordinary Address line 1 field, picks the right one from the suggestions underneath, and the rest of the address fills itself in. Without the module - or if the lookup service is having a bad day - the fields simply behave as they always have.

### Dressing the whole-basket promise

When another module works something out about the order as a whole - **Advanced Shipping**'s "Everything gets to you by Friday 4th", say - the shop prints it verbatim. It never writes the sentence itself. What it *does* decide now is how that sentence looks, and it decides it separately on each of the two places it appears: the cart page and the checkout order summary. (The slide-out basket used to be a third. It no longer prints the note at all - see [The slide-out basket](#the-slide-out-basket).)

Each of those is a block, and each block carries the same set of **Basket note** settings:

- **Style** - four choices. *Plain line* is the modest one-liner it has always been. *Coloured box* puts it in a filled panel with corners you choose. *Image and speech bubble* pairs a picture of your choosing with the note coming out of its mouth, tail and all. *Hidden* takes it off that screen entirely.
- **Background**, **text** and **border** colours, each a swatch from your own palette with its own dark-mode row underneath, so you set light and dark in one go and there is no second set of settings to keep in step.
- **Corner radius** and **text size**.
- For the speech bubble: the **picture** itself (chosen from your media library), how **wide** it should be, whether it sits **left or right** of the bubble, and how big the **point** is.
- **Tick in front** and **bold text** for the plain line.

The box and the bubble both fill the whole width of whatever column they are in, which on a desktop checkout is a good deal more room than the old grey line was using. They shrink to suit: in a narrow column the type and the picture come down a size, and in a very narrow one the two stack, the bubble taking the top with its point swung down at the picture below. Nothing is left running off the side of a phone.

Set it twice, differently, and nothing objects - a cheerful mascot at checkout, nothing at all on the cart page. The checkout summary starts out looking exactly as it always has; the cart page starts out with the note in the sticky bar only, as before, so switching it on there is something you choose rather than something that happens to you.
...
Picking a payment method is now just that - picking one. The methods that finish somewhere else (PayPal, Instant Bank Pay) used to whisk the shopper off the moment they touched the radio button, before they'd so much as glanced at the total. Now nothing happens until **Place order** is pressed, and that button is the only thing that hands anyone over. Coming back afterwards without having paid starts the payment again properly rather than quietly waving the order through to the thank-you page, which is the sort of tidiness you only notice when it's missing. And paying that way now empties the basket on the way through, so nobody is thanked for an order they're apparently still carrying. The methods that finish on somebody else's site - Instant Bank Pay, Pay by bank (Atoa) and Card payment (Square) - go one better and no longer write the order down at all until the payment is committed, so a checkout abandoned on the bank's page leaves nothing in your orders list to puzzle over later.

**A word of warning before the lights come on.** A shopper browsing your site in dark mode and paying by a method that finishes elsewhere - Instant Bank Pay, Pay by bank (Atoa), Card payment (Square), PayPal - now gets a short notice before they are handed over: the payment provider draws its own site in its own colours, and almost none of them do dark, so an unwarned shopper gets a screenful of white at the exact moment they are being asked to approve money. The notice says where they are going, shows a bar draining away, and sends them on after five seconds whether or not they touch it - there is a **Continue** button for anyone who would rather not wait. Nobody on the light version of your site sees any of this, and nothing about the payment itself changes.
...
Sell to businesses and you'll want to know who you're selling to. **Settings → Shop → Checkout → Organisation name** adds a box for it, sitting directly under the customer's own name on the contact step.

It used to sit inside the delivery address, above the first line, and that was the wrong place for it. An organisation says who somebody is, not where a parcel goes - so it was being asked for again on every new address, and a customer with three offices ended up with three copies of the same company name. It's now asked once, kept on their account, and filled in for them next time. Anyone who does want the company printed on the delivery label puts it in the first line of the address, which is where a courier reads it anyway.
...
- The customer can **add it themselves afterwards**, where you've switched that on, from their own order page.
- It's on their **order list and their receipt**, so a business buyer can find last month's order by their number rather than by yours.
...
A signed-in customer with addresses saved to their account gets to pick from them here, under **Bill to**, the same way they pick where the parcel goes - the accounts department is usually somewhere they've had something delivered before, and typing it out a second time on the same page is the sort of thing that loses an order. Picking one takes the address only: the name and the phone number stay behind, because this form has no boxes for them. **Use a different address** brings the empty form back. A saved address that turns out to be short of a town or a postcode brings the form back out ready-filled, rather than going onto the invoice half written.

A billing address they typed rather than picked joins their address book once the order is placed, exactly as the delivery address does, filed under **Billing address** so it's tellable apart in a list of doors. It never becomes their default, since the default is what the next checkout offers to *deliver* to. Bill to the same place the parcel goes and nothing new is filed - it's the same address, and one entry is quite enough.
...
It also, quietly, reports the sale. Nothing in the shop knows or cares who is listening - if you have installed something that measures your site, such as [Google Tag](Google-Tag), it hears the order number, the total, the tax, the delivery, the coupon and every line, and does whatever it does with them. If you have not, nothing happens at all. An order is reported when it is paid, or when it has been placed on a bank transfer or cash and is waiting for the money; a payment still going through, or one that failed, is not reported. It is reported exactly once, however many times that page is refreshed or revisited.
...
What slides in is the real basket, not a teaser. Every item with its photo, its chosen options, the minus/plus stepper and a **Remove** link, and - if you run **Advanced Shipping** - the same delivery service picker each item has on the cart page, so a shopper can move something to a faster service without ever leaving the product they were reading. A panel is a narrow thing, so it arranges those pieces to suit itself: the price, the amount and the **Remove** link stack in one tidy column down the right, all the same width, and the delivery service sits across the full width underneath the photo and the name where its wording has room to breathe. The cart page itself is unchanged. Underneath the items: the subtotal and the VAT side by side on one line, the total beneath them, a **Checkout** button and a **View full basket** button for anyone who wants the roomier page.

**It gives the room to the basket.** A panel on a phone is not a tall thing, and it used to spend a good share of itself on everything but the items. The heading bar is now barely taller than its close button. The subtotal includes the delivery the shopper picked - which is already printed on the item it belongs to - so there is no separate delivery line to repeat it. And the whole-basket sentences - the delivery module's "Everything gets to you by Wednesday" and the order-size deduction's "Add £252 more from…" - stay on the cart page and at checkout, where there is room for them, rather than squeezing the basket off the top of the panel.

**The chat bubble steps aside.** With the [Live Chat](Live-Chat) block on your site, its floating bubble used to sit squarely over the **View full basket** button whenever the panel was open. It now disappears while the basket is out, and the chat is offered inside the panel instead: a full-width button, in the bubble's own colours and wording, under **View full basket**. Pressing it closes the basket and opens the chat. Shut the basket and the bubble comes back. Remove something by mistake and the usual **Undo** message appears, exactly as it does on the cart page.
...
**Up from the bottom, full width.** **Slide-out: opens from** has a third choice - **Bottom (full width)** - and it is the one to pick for phones. The panel comes up from the bottom edge across the whole width of the screen, with rounded top corners, rather than squeezing the basket into a narrow column down one side. Everything inside it is unchanged: the same lines, the same delivery pickers, the same buttons. The width setting is simply ignored, there being nothing to be narrower than. This is also what the basket cell in a [Mobile Bar](Appearance-and-design#a-bottom-bar-for-phones) opens.
...
Switching the whole thing back off hides the Suppliers tab and the boxes, but keeps every supplier and every name you'd recorded, ready for when you change your mind. The same goes for narrowing the field back to products only: variation suppliers are left where they are, not scrubbed.
...
**Shop → Trading → Suppliers** is the address book behind that box. Each entry holds a name, your account number with them, the trade discount you're on, whether they're enabled or disabled, a contact person with their phone number, email address and postal address, and a notes field for everything that fits nowhere else.
...
**The supplier's name is a link.** Where supplier pages are switched on and that supplier's page is published, their name in the line - on the product page, in the cart page's note and in the checkout summary's - links to their page, so "Dynamic Office Solutions orders of £389 or more" is one click from everything else they make. It opens in a new tab, so nobody loses the product or the basket they were halfway through. With no published page the name is plain text, as it was.
...
Plenty of shops sell other people's ranges, and shoppers who care about that ask for it by name: "what have you got from Senator?" Until now the only honest answer was a search box. Suppliers can now have a page each, listing everything of theirs with your filters down the side and room for a proper write-up above.

It takes two switches, on purpose. **Give suppliers a page of their own** under **Shop → Settings → General** decides whether such pages exist at all, and it's off to begin with - switching supplier support on has never meant publishing anything, and it still doesn't. Then each supplier is published individually from the **Suppliers** screen, so you can write one up properly this afternoon and leave the other forty as filing.

Open a supplier and there's a **Their page on your site** section holding the lot:

- **Give them a page** - the tick that publishes it. Untick it and the page goes, while everything you'd written stays where it was.
- **Web address.** The page lives at `/shop/suppliers/their-name`. It's yours to type rather than something that quietly follows the name around, because changing an address stops old links working - the same arrangement collections have. Leave it empty and their name is used.
- **One-line description** - printed under the heading.
- **Write-up** - a paragraph or two about them. And if a paragraph in a box isn't enough, **Design it with the page builder** opens the full-screen builder in a new tab, where the write-up can have pictures, columns and callouts in it. Anything you build there is shown instead of the plain version, and a supplier with one is marked **Designed** in the list.
- **Search engine title and description** - how the page reads in results. Blank falls back to their name and your one-liner.

The list itself grows a **Page** column with a **View** link on every supplier that has one, so checking what a shopper sees is a click.
...
Published supplier pages join your sitemap too, but only where the supplier has something on sale. An empty page is a thin page, and there's no sense advertising it.
...
When the shop status is set to closed, visitors see your closed message instead of the shop. That covers every shop page, not just the front one: product pages, categories, collections, the cart and the checkout all show the closed message too. Closed means closed, including to somebody who kept a link to a particular product and tried it directly.

What stays open is everything to do with orders people have already placed: tracking, the receipt and order pages, delivery updates, live delivery, their paperwork and the downloads they paid for. Closing for a fortnight's holiday shouldn't leave last week's customers unable to find out where their parcel is.

Anyone signed in with Shop access still sees the whole shop as normal, with a note at the top of each page reminding them it's closed to everyone else, and can go right through the checkout, so you can walk the place over before you reopen.
