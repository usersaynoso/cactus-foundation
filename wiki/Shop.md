...
**When the next van is due.** On the orders list, the Dispatch column also shows **Delivery due Tue 22 Sep** under the badge - the soonest delivery day booked on any parcel that hasn't arrived yet. A parcel that has already been delivered is skipped, so an order split across Tuesday and Friday reads Tuesday until Tuesday's has landed and Friday after that. It comes from the **Delivery date** typed into **Dispatch items** (or edited on the parcel afterwards), so a parcel with no day booked shows no line.
