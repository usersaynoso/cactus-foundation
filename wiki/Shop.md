...
The **description, specification and downloads** piece has grown a few choices of its own. Out of the box it shows them as tabs the shopper clicks between; you can instead have them **stacked** one under another with everything on show, or as an **accordion** where each heading opens and closes on a tap. When you keep the tabs, you can line them up **left, centre or right**, and make the row **stick** to the top of the screen as the shopper scrolls, so the section links are always to hand. There's also a **divider** switch for the faint line and gap above the piece - handy to turn off if you move it to the very top of the page, where that gap would otherwise sit there looking like a mistake.

If you'd rather the section links sat somewhere else entirely - above the photo, say, or across the top of the page - drop in the separate **Section links** piece and put it wherever you like. It shows the same Description / Specification / Downloads links, and tapping one jumps down to that section. Pair it with the main piece set to **stacked** or **accordion** so there's always a section for each link to land on.
