...
Cards and product pages both print every badge a product has earned, not just one - so a reduced chair that is also flagged Ex-display says both, in the grid and on the page it leads to. On a card they stack down the top-left corner of the photo in that order. "Out of stock" and "Pre-order" come first whatever else is true, then your own tag badges, then the built-in ones. If you'd rather not see a particular kind at all, the **Card: Badge** part in your Product Card layout has a switch for each.
