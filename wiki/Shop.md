...
When another module works something out about the order as a whole - **Advanced Shipping**'s "Everything gets to you by Friday 4th", say - the shop prints it verbatim. It never writes the sentence itself. What it *does* decide now is how that sentence looks, and it decides it separately on each of the three places it appears: the slide-out basket, the cart page and the checkout order summary.
...
Set it three times, differently, and nothing objects - a cheerful mascot at checkout, a plain green tick in the slide-out, nothing at all on the cart page. The slide-out and the checkout summary start out looking exactly as they always have; the cart page starts out with the note in the sticky bar only, as before, so switching it on there is something you choose rather than something that happens to you.
...
What slides in is the real basket, not a teaser. Every item with its photo, its chosen options, the minus/plus stepper and a **Remove** link, and - if you run **Advanced Shipping** - the same delivery service picker each item has on the cart page, so a shopper can move something to a faster service without ever leaving the product they were reading. A panel is a narrow thing, so it arranges those pieces to suit itself: the price, the amount and the **Remove** link stack in one tidy column down the right, all the same width, and the delivery service sits across the full width underneath the photo and the name where its wording has room to breathe. The cart page itself is unchanged. Any whole-basket promise your delivery settings work out ("Everything gets to you by Fri 4 Sep") sits above the total - in green with a tick to begin with, though the widget's own **Slide-out: basket note** settings will turn it into a coloured box, a picture with a speech bubble, or nothing at all (see [Dressing the whole-basket promise](#dressing-the-whole-basket-promise)). Underneath: the subtotal, a **Checkout** button and a **View full basket** button for anyone who wants the roomier page. Remove something by mistake and the usual **Undo** message appears, exactly as it does on the cart page.
