...
- **On your own checkout** (the recommended setting) draws Square's own card fields underneath the payment method, on the page the shopper is already looking at. The fields are Square's, served from Square and sealed off from the rest of the page, so the card number is no more yours than it was before - the shopper simply never leaves. Pressing **Place order** takes the payment then and there. This needs the Application ID as well as the access token, and the method is quietly not offered until that box is filled in, on the grounds that a card form that cannot be drawn is worse than one that was never promised.
