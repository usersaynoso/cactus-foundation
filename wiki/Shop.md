...
**When the courier could not deliver.** Couriers do not always get in. Each courier you follow automatically has a third list alongside the out-for-delivery and arrived ones: **stages that mean the delivery failed**. The start of the courier's wording is enough - "Failed Attempt" catches "Failed Attempt - Non Fault - Recipient Not Home" and whatever reason they staple on next time. When a parcel reaches one, the customer's order page stops saying "Out for delivery", the van and the map go, the rail reads **Delivery not possible**, and in place of the booked day the customer is told to get in touch with the courier to arrange a new one - by the **live chat link** and **phone number** you give that courier, with their tracking number printed alongside, because it is the first thing the courier will ask for. They are also sent one email saying the same - **Delivery not possible** under Settings → Emails, where you can reword it. One per failed attempt: a second failure on the same parcel is a second email, the hourly re-check of the same failure is not. The courier's own reason is not repeated to the customer: it is written for drivers, and "Recipient not home" reads as an accusation to somebody who was in the garden.

Sometimes you have already rung the courier and they have said *they* will call the customer. On the order screen, a failed parcel shows what the customer is being told and a button to change it: **[Courier] will contact the customer**. Press it and the customer's page says the courier is arranging a new day and will be in touch, and that there is nothing they need to do; press it again to go back. It clears itself the moment the courier's tracking moves on - a new day booked, or another failed attempt - so an old "they will ring you" never hangs over a new problem. The button sends nothing itself. The email usually goes before anybody has had the chance to press it, so it will normally be the "get in touch" version; if the button was pressed first, the email says to wait instead.
