...
- **Details** - the name, the descriptions, and your own SKU and barcode. If you've switched on suppliers (see below), the box for picking one lives here too. The web address is set when you first save and left alone after that; rebuilding it to match a new name is a button on the **Search** tab. The full description can stay plain text, or you can hit **Design with the page builder** to lay it out with headings, images and columns instead - and once you're designing, **Open in a new tab** pops that builder out into a page of its own with nothing else on screen, handy on a small window. Saving there sends the design straight back to the product; it never fights with anything you're doing on the main page.
