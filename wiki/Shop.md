...
**Extra charges - a redelivery fee, say.** When a courier charges you for a second attempt because nobody was in, you can pass that on. On the order screen, **Extra charges → Raise a charge**: say what it is for (the customer sees this - "Redelivery fee"), the amount before VAT, the VAT rate (it starts at the order's own rate) and, if you like, a note. By default the order goes **on hold** and the customer is emailed (**Extra charge to pay** under Settings → Emails).

On their order page the customer then sees the charge with two ways out of it:
...
- **Cancel instead**, and be refunded what they paid **less the charge** - the attempt that already happened cost you exactly that. The refund goes straight back the way they paid, the order is cancelled, both of you are emailed, and the credit note is for the smaller amount, so the fee stays on the invoice, VAT and all. Getting the goods back from the courier is up to you.

From the order screen you can do either on the customer's behalf (for the customer who rings rather than clicks), send the email again, or **Waive** the charge. One charge can be waiting on an order at a time. A charge that is paid is paid separately from the order, so it is not on the order's invoice - a note on the order's history says so, and it wants recording in your books.
