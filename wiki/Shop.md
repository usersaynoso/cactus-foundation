...
**Product page address.** If you would rather a product lived at the very top of your site - `/your-product` instead of `/shop/products/your-product` - flip **Product page address** in **Settings → Shop → General → SEO**. Only the product page itself moves; categories, collections, the cart and checkout all stay under `/shop`. Each product then has exactly one address: the old `/shop/products/…` one stops existing and returns a not-found page, so there is no chance of the same product sitting at two addresses and search engines having to pick. Everything on your own site follows the setting on its own - the sitemap, menus, product grids, search results, reviews, the accessories panel and the Google Shopping feed - but anything pointing at the old address from **outside** your site (a link a customer bookmarked, an old post elsewhere, a search result not yet recrawled) will land on a not-found page, so it is worth switching deliberately rather than on a whim. If you use the site search, run a reindex afterwards so its results carry the new addresses. A page of your own with the same address always wins the spot - the shop only answers where nothing else does (technically: core resolves an info page first, then a module index, then a module's `publicRootSlug` claim; shop claims a bare slug only when the style is set to root and a product row carries that slug). The sitemap, menus, product grids, and the Google Shopping feed all follow the setting on their own.
