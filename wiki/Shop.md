...
What slides in is the real basket, not a teaser. Every item with its photo, its chosen options, the minus/plus stepper and a **Remove** link, and - if you run **Advanced Shipping** - the same delivery service picker each item has on the cart page, so a shopper can move something to a faster service without ever leaving the product they were reading. Any whole-basket promise your delivery settings work out ("everything by Fri 4 Sep") sits above the total, in green, with a tick. Underneath: the subtotal, a **Checkout** button and a **View full basket** button for anyone who wants the roomier page. Remove something by mistake and the usual **Undo** message appears, exactly as it does on the cart page.
