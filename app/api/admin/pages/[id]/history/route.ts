// Version history endpoint for Puck builder pages.
// Requires pages.read.
//
// Addressing scheme:
...
export async function GET(request: NextRequest, { params }: Params) {
  ...
  if (indexParam !== null) {
    if (indexParam === 'live') {
      if (!page.publishedData) return errorResponse('No published version exists', 404)
      return NextResponse.json({ data: page.publishedData })
    }

    const n = parseInt(indexParam, 10)
    if (isNaN(n) || n < 0) return errorResponse('Invalid index', 400)

    const history = Array.isArray(page.history) ? (page.history as HistoryEntry[]) : []
  ...
  if (page.publishedById) userIds.add(page.publishedById)
  ...
  if (page.publishedData) {
    versions.push({
      index: 'live' as const,
      at: page.publishedAt?.toISOString() ?? null,
      title: page.title,
      byName: page.publishedById ? (nameMap.get(page.publishedById) ?? null) : null,
      isLive: true,
    })
  }
