...
const AppearancePuckEditor = dynamic(() => import('../AppearancePuckEditor'), {
  ssr: false,
  loading: () => <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', color: '#6b7280' }}>Loading header editor…</div>,
})
...
export default function AppearanceHeaderPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/appearance')
      .then((r) => r.json())
      .then((d) => {
        setInitialData((d.headerBuilderData as Data | null) ?? { content: [], root: { props: {} }, zones: {} })
        setLoading(false)
      })
      .catch(() => { setError('Failed to load header data'); setLoading(false) })
  }, [])
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch('/api/admin/appearance', {
          ...
          body: JSON.stringify({ headerBuilderData: data }),
        })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {initialData && <AppearancePuckEditor mode="header" initialData={initialData} onChange={handleChange} />}
    </div>
  )
}
