...
for (const moduleName of moduleNames) {
  ...
  for (const entry of extensionPoints) {
    ...
    byPoint.get(entry.point).push({ id: entry.id, ident, component: entry.component, admin: isAdminOnly(importPath), importPath })
  }
}
...
// ── the public half ────────────────────────────────────────────────────────
// Same map, minus every entry whose component lives under components/admin/.
...
pub.push(`//`)
pub.push(`// The same map as extension-points.ts with every components/admin/ entry left`)
pub.push(`// out. Import THIS one from anything a public page can reach.`)
pub.push(`//`)
...
const adminDropped = entryCount - publicImports.length
console.log(
  `[generate-module-extension-points] extension-points.ts written (${entryCount} entr${entryCount === 1 ? 'y' : 'ies'} across ${pointCount} point${pointCount === 1 ? '' : 's'}); extension-points.public.ts written (${adminDropped} admin entr${adminDropped === 1 ? 'y' : 'ies'} withheld)`
)
