...
- `boards.access` - the Boards admin sidebar section (Threads/Structure/Moderation/Moderators/Analytics nav entries) and baseline module admin screens.
...
Admin pages (`/cactus-admin/m/boards/…`, tab bar `BoardsNav.tsx`): `threads` (the manifest nav-entry landing page - list/filter/bulk moderation), `structure` (sub-tabbed via shared `TabStrip`: Categories/Boards/Sub-boards/Tags/Templates, `boards.manage`), `moderators` (isAdmin only), `moderation` (sub-tabbed via shared `TabStrip`: Queue/Reports/Bans/IP Bans/Log), `analytics` (`boards.access`). `settings` and `import` pages removed - moved into a `settingsTabs`-registered "Boards" tab on core `/cactus-admin/config` (`BoardsSettingsTab.tsx`, replacing the old `SettingsScreen.tsx`), with the import wizard appended below it, same reshuffle Gazette got.
