...
- `MEMBER_AREA_PATH` - optional; public URL prefix for the member account area (default `account`). Deploy-time only, changing it requires a redeploy. Read via `getMemberAreaPath()` in `lib/members/paths.ts` (sanitised, blocklist-checked, falls back to the default). Member-area pages will live internally at `/cactus-account/*` (rewrite lands in a later Members phase).
...
- `Member` - `id`, `email` (unique), `username` (unique, stored lowercase), `displayName?`, `avatarMediaId?` (Media id, no relation), `avatarChoice AvatarChoice` (GENERATED), `bio?`, `websiteUrl?`, `trusted` (false), `status MemberStatus` (PENDING_VERIFICATION), `emailVerified` (false), `emailVerifiedAt?`, `backupEmail?`, `suspendedUntil?`, `suspensionReason?`, `suspensionNotified` (false), `deletionRequestedAt?`, `deletionScheduledAt?`, `deletionExportReady` (false), `usernameChangedAt?`, `previousUsername?`, `previousUsernameExpiresAt?`, `createdAt`, `updatedAt`. Indexes: status, previousUsername, deletionScheduledAt, createdAt.
