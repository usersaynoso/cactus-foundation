...
- `/cactus-admin/roles` - role list + permission matrix (`RolesClient.tsx`); protected role shows all-granted and is immutable; permissions grouped Core vs module, inactive-module groups disabled. Server component also renders any `core.roles-page` extension-point contributions below the matrix (permission-filtered live from `Module.manifest.extensionPoints`, same pattern as elsewhere) - currently unused by any shipped module: Gazette's Contributor/Author/Editor tiers and Boards' moderator status both moved *into* the permission matrix itself (`gazette.contributor`/`gazette.author`/`gazette.editor`, `boards.moderate`) rather than living in a bespoke `core.roles-page` card, so there's nothing left for the extension point to render unless a future module needs it.
