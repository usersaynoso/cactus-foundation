...
- **Before saying "no module updates", sweep module repos in every Cactus checkout you can see, including nested checkouts.** This tree may contain another full `cactus-foundation` checkout at `cactus-foundation/`; its `modules/*` repos are independent from the outer `modules/*` repos and can hold the real dirty module work. Check both with `git -C <checkout>/modules/<name> status --short`, then release every dirty module and finish with the core `modules.json` pin release.
