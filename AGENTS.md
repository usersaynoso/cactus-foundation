...
- **STRICT: one checkout only - `/Users/chris/Git Local/Cactus`. Never clone, copy, or create a second full checkout of this repo (e.g. a `cactus-foundation/` directory) anywhere inside or alongside it.** A nested clone is confusing, not a feature - it splits git history across two places, drifts out of sync, and its "uncommitted work" is nearly always stale/already-shipped once you actually diff it against the real checkout. If you ever find one (however it got there), do NOT sweep it as routine infra: diff every dirty file against the primary checkout and against each module's own remote first. Anything that's genuinely new gets committed from the primary checkout's paths (or the module's real dev clone under `modules/<name>/`), never released from inside the nested copy. Once nothing unique is left in it, delete the whole nested directory so there is exactly one source of truth again - don't leave it sitting there "just in case".
...
- **`next` must stay at `^16.3.4` or later.** Next 16.2's Turbopack holds the whole module graph in memory and, on a build machine with 8 GB, goes over the ceiling. It does not fail cleanly: it thrashes, prints nothing after `Creating an optimized production build ...`, and Vercel kills the deployment at 45 minutes with the site not updated. That is exactly what happened on 2026-09-02, the first deploy after the build machine was dropped to 4 cores / 8 GB. Next 16.3 is where the Turbopack memory work landed. `scripts/next-build.mjs` is the belt to that braces: it kills a build that has gone silent for 10 minutes or run for 25, and retries once cold with `CACTUS_TURBOPACK_BUILD_CACHE=0`.
...
- **Nested checkout gotcha:** if `/Users/chris/Git Local/Cactus/cactus-foundation` exists, it is its own Cactus core checkout, with its own `modules/*` module repos. A commit-and-push run from the outer repo must not treat the outer `modules/*` sweep as proof that the nested checkout's modules are clean.
