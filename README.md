...
- [Google Reviews for Shop](https://github.com/cactus-foundation-modules/google-reviews-for-shop) - finds your products on Google Shopping and brings the reviews people have already written about them into your own Reviews list, one product or a big batch at a time. Every one is labelled so nobody mistakes a stranger's words for a customer's, and the looking is done by a small browser machine of your own at Fly.io - set up from the settings screen with one pasted token, kept on a fixed address, and put away automatically when idle. Requires the Shop and Reviews modules.
