...
- [Product Discovery Tool](https://github.com/cactus-foundation-modules/product-discovery-tool) - a guided buying flow for shoppers who do not yet know what to tick. They pick what sort of thing they are after, then what sort of that, then what matters to them, and the products narrow as they go. Every option along the way says what it means, what it is best for and what to watch out for, with a like-for-like comparison of the ones beside it - so somebody who cannot tell a cantilever frame from a panel end still buys the right desk. Runs on the filters you already have, so nothing is set up twice. Requires the Shop and Filters for Shop modules.
- [Product 3D Views for Shop](https://github.com/cactus-foundation-modules/product-3d-views-for-shop) - puts a slowly turning 3D model in the product gallery beside the photographs; shoppers click it to turn, pan and zoom the product. Models attach to a product or to individual variations. With the Google AI Studio module installed it also poses for photographs: turn the model to any angle and send that view over as a reference picture. Requires the Shop module.
...
- [Product Add-ons for Shop](https://github.com/cactus-foundation-modules/product-addons-for-shop) - offer other products as add-ons right on a product's page: screens with a desk, extra shelves with a cupboard. Sizes match themselves to what the shopper has configured, colours follow their choices unless overridden, sensible quantities are suggested, and everything lands in the basket as one tidy group - each piece still priced, stocked and delivered by its own rules. Call them accessories, add-ons or anything else sitewide. Requires the Shop and Shop Variations modules.
- [Product Checks for Shop](https://github.com/usersaynoso/product-checks-for-shop) - a private notebook for shop staff: record when each part of a product listing - words, pictures, 3D models, variations, downloads - was last given a proper look, and see at a glance which products have gone longest without one. Shoppers never see any of it. Lives in its author's own account rather than the module directory, as any custom module can - install it by pasting its address. Requires the Shop and Shop Variations modules.
- [Reviews for Shop](https://github.com/cactus-foundation-modules/reviews-for-shop) - customer reviews on your product pages: shoppers rate a product out of five and say why, you publish each one (or let them go up on their own), reply underneath, badge the ones from real customers, and optionally email past buyers to ask. Requires the Shop module.
...
- [Square Payments for Shop](https://github.com/cactus-foundation-modules/square-payment-for-shop) - adds card payment at checkout via Square's hosted checkout page, so card details never touch your site, with refunds and automatic confirmation. Requires the Shop module.
- [Pay With Atoa for Shop](https://github.com/cactus-foundation-modules/pay-with-atoa-for-shop) - adds pay by bank at checkout through Atoa: a one-off, open-banking payment the shopper approves in their own banking app, with no card and automatic confirmation. Refunds are recorded on the order and sent from your own bank, the same way a bank transfer is refunded. Requires the Shop module.
- [Advanced Shipping for Shop](https://github.com/cactus-foundation-modules/advanced-shipping-for-shop) - swaps vague "within a few working days" copy for a concrete, live delivery date on every product page and basket line, worked out from the supplier, category and range, a daily cut-off, and working days only (weekends and UK bank holidays skipped). Adds paid delivery-and-assembly service tiers the shopper picks per item. Requires the Shop and Product Attributes modules.
- [Address Lookup for Shop](https://github.com/cactus-foundation-modules/address-lookup-for-shop) - shoppers type the first line of their address at checkout and pick the right one from a list; the rest of the address fills itself in. Powered by Ideal Postcodes (bring your own API key), and the fields still work the ordinary way for anyone who prefers typing. Requires the Shop module.
...
- [Google Shopping for Shop](https://github.com/cactus-foundation-modules/google-shopping-for-shop) - puts your catalogue on Google Shopping: a Merchant Center feed in which every variation is its own listing, with its own price, photos, stock and link, grouped under its parent product. Paste one address into Merchant Center and Google keeps itself up to date; brand and barcode details set per product, and anything you'd rather keep off Google sits the feed out. Requires the Shop and Shop Variations modules.
- [Stock Imports for Shop](https://github.com/cactus-foundation-modules/stock-import-for-shop) - give it the web address of your supplier's stock file and every product whose code appears in it keeps its stock count up to date on its own, as often as you like. A Test button shows you how many of your products the file actually knows about before you commit to it, and a Fetch Latest Stock button on the Products page is there for when you would rather not wait for the schedule. Requires the Shop module.
- [Abandoned Carts for Shop](https://github.com/cactus-foundation-modules/abandoned-carts-for-shop) - shows you the baskets nobody finished, with whatever the shopper had typed into the checkout before they went: name, email, phone, delivery address, discount code. Optional reminder emails, worded in your own email settings, go to the ones who left an address, and the list says of each basket whether one went, when, and if not, why not. Records nothing until a shopper agrees to it on your cookie banner, deletes everything after a number of days you choose, and asks nothing of the Shop module itself. Requires the Shop module.
- [Quote for Shop](https://github.com/cactus-foundation-modules/quote-for-shop) - for shops that quote rather than sell off the shelf. Either turn the whole shop into "Add to quote" with no checkout and prices optionally withheld, or leave checkout alone and let shoppers park a basket as a quote, fetch it back with a short code, and download it as a PDF of a document you design yourself. Quotes land in an admin list where you price them, reply, send them out and turn the accepted ones into orders. Requires the Shop module.
- [Space Planner for Shop](https://github.com/cactus-foundation-modules/space-planner-for-shop) - shoppers draw their own room to scale, drop your catalogue into it in 2D and 3D, and find out what actually fits before they spend anything. Uses the 3D models you already have and shows everything else as a tidy labelled block at the right size, so the whole catalogue is plannable rather than the small modelled corner of it. Out the other end: a printable floor plan, a priced item list, a basket, or a quote request. Requires the Shop, Shop Variations, Product 3D Views and Quote modules.
- [Modular Configurator for Shop](https://github.com/cactus-foundation-modules/modular-configurator-for-shop) - shoppers build a layout from a modular product's units - a sofa that turns a corner, a run of bench seating - in 3D on the product page, see how much floor it takes, and buy the whole arrangement in one go as a tidy group of basket lines. Units only join the way they physically can: arms at the ends, corners turning the right way, nothing overlapping. Requires the Shop, Shop Variations and Product 3D Views modules.
...
- [Google AI Studio](https://github.com/cactus-foundation-modules/google-ai-studio) - one Google AI Studio key, and the site can make pictures for you and draft your replies. With the Shop module installed, a product's Images tab grows an AI photo section: tick the photographs you already have - the product's own, its variations', any picture attributes on it - say what you would like, and pick from what comes back. With the 3D Views module installed you can add angles captured off the product's own 3D model too. With the Unified Inbox or the Contact Form installed, every reply box grows a Suggest reply button that reads the conversation and offers three drafts; click one to try it in the box, tick to keep it or cross to put back what you were writing. Nothing reaches your media library until you choose it, nothing reaches the product until you save it, and no draft is sent until you send it.
...
- [UK Bookkeeping](https://github.com/cactus-foundation-modules/uk-bookkeeping) - keep the books on your own site and file your VAT return straight to HMRC. Records income and expenses with the receipts attached, works out all nine VAT boxes from those records - you never type one - and locks everything behind a return once it has been filed. Behind that sits one proper set of books, so it will also give you a profit and loss account with last year beside it, a balance sheet that adds up, and the history of any single account down to the receipt. Reads a bank statement, as a PDF or a CSV, and ties it back to what you have already recorded so you can see whether the bank and the books agree. Keeps a register of your equipment, spreads its cost over the years you use it, and works out the tax allowances on it. Closes the year off when you are ready, and then works out the corporation tax - add-backs, allowances, losses, the small profits rate and marginal relief - and hands you every box number for the company tax return. Keeps journals for the things that are not money moving, and a director's loan account for money you put in and take out. Corrections go on the next return, exactly as HMRC expect. Works perfectly well without an HMRC connection if you file some other way.
- [Purchase Orders](https://github.com/cactus-foundation-modules/purchase-orders) - the buying side of the business. Keep a list of who you buy from and the terms you buy on, raise a purchase order against it, have the big ones approved before they go out, and keep an unarguable record of every change. Works on its own; works better with the Shop module, which lets you pick lines straight off your catalogue, and with UK Bookkeeping, which takes the bills through to the books. Check deliveries in, send goods back, and match a supplier's invoice against what you ordered and what turned up before anybody agrees to pay it. Reorder levels raise the drafts for you, suppliers get a link to their own order, and the reports tell you what you have committed to, what is late and what you are spending with whom.
- [Unified Inbox](https://github.com/cactus-foundation-modules/unified-inbox) - every conversation with every customer and supplier in one place: email, live chat, contact form enquiries, calls and texts, with your own orders, purchase orders, bills and quotes sitting beside them. Reads your real mailbox, answers as the address the customer wrote to, and collapses two emails, a chat and a phone call from the same person into one story. Works on its own as a proper email client; add the Live Chat, Contact Form or Twilio modules and their conversations turn up in the same list. Not a CRM, and staying that way.
