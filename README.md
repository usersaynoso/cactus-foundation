...
- [Crezco Direct Bank Payments for Shop](https://github.com/cactus-foundation-modules/crezco-direct-bank-payments-for-shop) - adds Direct bank payment at checkout: a one-off, open-banking (pay-by-bank) payment authorised on Crezco's hosted page, with no card and no card fees, confirmed automatically once the money moves. Refunds are handled outside Crezco (open banking has no refund rail). Requires the Shop module.
